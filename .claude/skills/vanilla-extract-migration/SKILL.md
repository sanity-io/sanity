...
Read theme values with `useTheme_v2()` (which returns the v2 theme object directly) and inject them via `assignInlineVars()`:
...
**Important:** `useTheme_v2()` returns the v2 theme directly (with `space`, `container`, `color`, etc.). Do NOT use `useTheme()` + `getTheme_v2(theme)` — that's the old pattern from styled-components prop access.
...
- **Never remove existing comments** when replacing styled components. Preserve all JSX prop comments, TODO notes, and explanatory comments from the original code.
- **Never remove existing props** — only change what's needed for the styling migration.
- **Always prefer classNames over inline styles.** Use `styleVariants` for fixed sets of values that change based on props/state. Only use `assignInlineVars` + inline `style` as a last resort for truly dynamic values (e.g. pixel values from calculations, theme-derived numbers). If the values are from a known fixed set, use `styleVariants` instead.

```ts
// ✅ Good — fixed values, use styleVariants
export const cursor = styleVariants({
  grid: {cursor: 'move'},
  list: {cursor: 'ns-resize'},
  disabled: {},
})
// <div className={readOnly ? cursor.disabled : cursor.grid} />

// ✅ Good — truly dynamic value, use createVar + assignInlineVars
export const widthVar = createVar()
export const root = style({width: widthVar})
// <div className={root} style={assignInlineVars({[widthVar]: `${computed}px`})} />

// ❌ Bad — fixed values via inline style
// <div style={{cursor: readOnly ? 'default' : 'move'}} />
```
...
| Wrong theme access pattern                    | `useTheme()` + `getTheme_v2(theme)`  | Use `useTheme_v2()` directly — returns v2 theme       |

...
| `bisectSession` | Bisect tool (user-owned, liveEdit)             | `bisect-session-<uuid>`                         | `description` (the issue), `refines` (weak ref to the session it narrows down — a chain counts as one regression), `good/bad{sha}`, `releasesOnly`, `reproPath` (test-studio path appended to every preview URL), `marks[]`, `result{firstBadSha, suspectShas, regression, linearIssue, fixedIn}`                                                                                                                                                                                |

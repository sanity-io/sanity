...
| `gitTag`        | same, releases and a daily npm floor           | `git-tag-v6-10-1`                               | `tag`, `sha`, `taggedAt`, `major/minor/patch/prerelease`, `npm{distTags, weeklyDownloads, publishedAt, deprecated}`                                                                                                                                                                                                                                                                                                                                                              |
| `bisectSession` | Bisect tool (user-owned, liveEdit)             | `bisect-session-<uuid>`                         | `description` (the issue), `refines` (weak ref to the session it narrows down — a chain counts as one regression), `good/bad{sha}`, `releasesOnly`, `reproPath` (test-studio path appended to every preview URL), `marks[]`, `result{firstBadSha, suspectShas, regression, note, severity (minor/major/critical), linearIssue, fixedIn}`                                                                                                                                         |
| `releaseLine`   | Releases tool (user-owned, liveEdit)           | `release-line-v<major>`                         | `major`, `eolMarkedAt`, `eolMarkedBy`, `note` — existence means the major is end of life; reinstating deletes it                                                                                                                                                                                                                                                                                                                                                                 |

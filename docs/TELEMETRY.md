...
| `Workspace Features Observed` | Fires once per workspace mount with a low-cardinality snapshot of the workspace feature flags (releases, tasks, scheduled drafts/publishing, media library, canvas, variants, events API, announcements, drafts, partial indexing, direct uploads, search strategy, advanced version control) |

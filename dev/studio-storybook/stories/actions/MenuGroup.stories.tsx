...
const meta: Meta<typeof MenuGroup> = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          "MenuGroup is the grouping primitive Studio's menus use to organize items into " +
            'sections, rather than one column that grows unbounded as features ship.',
          '',
          '|                 |                                                                                                                                                                             |',
          '| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |',
          ...
          '',
          'Drop one among a set of `MenuItem`s and its children live a level deeper, so the top ' +
            'of the menu stays short and scannable while Export, Advanced and Danger zone wait ' +
            'behind a hover.',
          '',
          'Open any menu below and hover a group to expand it. Groups nest, as the `Nested` ' +
            'story shows, but two levels is the practical limit: past that a person stops knowing ' +
            'where they are in the tree, and the chunking that was meant to reduce the search ' +
            'cost starts adding to it.',
          '',
          '> **Why it matters:** placement is not optional here. `@sanity/ui` `MenuGroup` ships ' +
            '_no_ default flyout placement, so an unconfigured group inherits `Popover`’s ' +
          ...
          '',
          'The last story shows it in context: the "Anna Karenina" document-actions menu with ' +
            'its long tail chunked into Publishing, Translate and a critical Danger zone group.',
        ].join('\n'),
      },
    },
  },

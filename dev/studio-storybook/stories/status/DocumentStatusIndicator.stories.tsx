      ...
      description: {
        component: [
          ...
          'The tiny row of coloured status dots, the compact, label-less lifecycle signal seen ' +
            ...
            'label would not fit.',
          '',
          'Each dot maps a status to a badge colour: published to `positive`, draft to ' +
            '`caution`, and one dot per active release version toned by release type (`asap` / ' +
            '`scheduled` / `undecided`). The draft dot only shows when the workspace draft model ' +
            'is enabled; version dots only show for releases in the active-releases store, inert ' +
            'by default in this harness, so most stories render the published/draft matrix, while ' +
            'the `WithVersions` story seeds the store (`WithStudioProviders({releases})`) so all ' +
          ...
          '',
          'The last story shows it in context: the status dot scanned down a real author list, ' +
            'Austen published, Tolstoy edited, Lem draft-only.',
        ].join('\n'),
      },
    },
  },

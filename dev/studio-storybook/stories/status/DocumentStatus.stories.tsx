...
const meta: Meta<typeof DocumentStatus> = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          "The text label and timestamp are DocumentStatus's saving grace, but the leading " +
            'glyph is the same circle for every lifecycle state, told apart by hue alone; strip ' +
            'colour and Published and Draft are identical.',
          '',
          '|          |                                                                                                                                                                                                                                                                                                                                                                                                                                                              |',
          '| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |',
          ...
          '',
          'The multi-line readout of where a document sits in its lifecycle: a labelled line ' +
            'each for Published, Draft, and every active release version. Editors need to know ' +
            'whether what they are looking at is published, still a draft, or scheduled in a ' +
          ...
          '',
          'Renders one line per live state, Published, Draft, and one per active release ' +
            'version, each as a `ReleaseAvatar` glyph plus a translated "Published {date}" / ' +
            '"Edited {date}" phrase. The draft line only appears when the workspace has the draft ' +
            'model enabled (`document.drafts.enabled`), and version lines only render for ' +
            'releases present in the active-releases store. That store is inert by default in ' +
            'this harness, so most stories render the draft/published matrix; the `WithVersions` ' +
            'story seeds it (`WithStudioProviders({releases})`) so release lines render live, ' +
            'title, release-type glyph tone (`asap` / `scheduled` / `undecided`) and ' +
            'created/edited phrasing all resolved through the real `useActiveReleases()` path.',
          '',
          ...
          '',
          'The last story shows it in context: the full lifecycle readout under the Leo Tolstoy ' +
            'document header, published, draft-edited, and three release versions at once.',
        ].join('\n'),
      },
    },
  },

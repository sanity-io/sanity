...
const meta: Meta<typeof Tab> = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          'Tabs come into the picture more often than you might think, and Studio has one ' +
            'button behind all of them, so a field-group tab, a Review changes panel, and the ' +
            'Tasks sidebar all read as the same control rather than three components that happen ' +
            'to look similar.',
          '',
          '| | |',
          '|---|---|',
          ...
          '',
          'So if you are building any surface where one pane offers a handful of alternative ' +
            'views, this is the primitive you compose, and it already looks like the rest of ' +
            'Studio.',
          '',
          'That consistency is the point of the shadow. It takes a subset of `@sanity/ui` `Tab` ' +
            ...
            'pins padding to `2` and `muted` on, so you cannot accidentally ship a tab that is a ' +
            'little too tall or a little too loud. The added `iconRight` slot ' +
          ...
          '',
          '> **Why it matters:** padding and font size are deliberately *not* configurable ' +
            'here. If you need a different size you are reaching for the wrong component, every ' +
            'tab in Studio is meant to read the same, and that uniformity is what the shadow ' +
            'exists to guarantee.',
          '',
          'The page closes *in context*: a document pane header for the book *Anna Karenina*, ' +
            'its views, Editor, Preview, History, riding a `TabList` across the top of the pane ' +
            'and swapping the panel below without leaving the document.',
        ].join('\n'),
      },
    },
  },

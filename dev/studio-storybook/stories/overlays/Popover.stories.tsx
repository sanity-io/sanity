...
const meta: Meta<typeof Popover> = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          'Popover is what almost every floating thing in Studio, a dropdown menu, a hover ' +
            'card, an inline colour picker, sits on underneath. The audit finding is what happens ' +
            'in the gap before its content is ready to paint.',
          '',
          '|          |                                                                                                                                                                                                          |',
          '| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |',
          ...
          '',
          'The full primitive surface and consistent motion come for free. Because the shadow ' +
            'adds nothing but the `animate` default, its full prop surface is `@sanity/ui` ' +
            '`Popover`: `content`, `placement`, `fallbackPlacements`, `constrainSize`, ' +
            '`preventOverflow`, `portal`, and a child (or `referenceElement`) as the anchor.',
          '',
          'Current reproduces the empty flash, an open popover with no content. Recommended ' +
            'paints skeleton structure immediately so the surface never reads as blank.',
          '',
          '> **Why it matters:** a popover can open before its content is ready and flash an ' +
            'empty box for ~1s, the instant-gratification finding. Paint skeleton structure the ' +
            'instant it opens so the surface reads as content loading here, never as a blank ' +
            'void.',
          '',
          'The page closes _in context_: a document pane header for the book _Anna Karenina_ ' +
            "whose toolbar button opens a Popover of the document's info, the anchored-panel " +
            'pattern that menus and info cards are all built on.',
        ].join('\n'),
      },
    },
  },

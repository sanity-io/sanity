...
const meta: Meta<typeof TooltipOfDisabled> = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          "TooltipOfDisabled fixes a Tooltip's dead zone on a disabled button for a mouse, and " +
            'stops there: a disabled button dispatches no pointer events, so a Tooltip placed ' +
            'directly on it never fires and a disabled control has no way to explain why it ' +
            'cannot be used.',
          '',
          '|          |                                                                                                                                                                                                                                                                                                                                                                       |',
          '| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |',
          ...
          '',
          'Note the inverted `disabled` semantics: it disables the _tooltip_, not the control, ' +
            'so the stories model the real idiom `disabled={!controlIsDisabled}`. The Recommended ' +
            'pair keeps the hover tooltip but also wires `aria-describedby` to a visually hidden ' +
            'reason node, so the explanation is programmatic, not pixels-only.',
        ].join('\n'),
      },
    },
  },

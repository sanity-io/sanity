...
const meta: Meta<typeof TooltipDelayGroupProvider> = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          'A toolbar where every icon button waits its own 400ms before showing a tooltip reads as ' +
            'sluggish the moment a finger moves along the row. This provider is how Studio fixes that: ' +
            'once one tooltip in a group has shown, its siblings reveal instantly.',
          '',
          '| | |',
          '|---|---|',
          ...
          '| Patterns | `datatips` |',
          '',
          'Wrap a cluster of controls (a formatting toolbar, for example) in the provider: the first ' +
            'tooltip in the group still waits the full open delay, but once one is showing, moving ' +
            'between siblings reveals their tooltips instantly, no repeated 400ms wait per button. The ' +
            'provider drops the `delay` prop entirely; the value is fixed.',
        ].join('\n'),
      },
    },
  },

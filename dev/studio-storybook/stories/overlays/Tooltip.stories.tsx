...
const meta: Meta<typeof Tooltip> = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          'A tooltip is a supplement, never the only home for information a person must act on, and ' +
            'this page exists because Studio does not always honor that. The shadow itself is a ' +
            'careful, standardized wrapper; the audit finding is in what gets asked of it.',
          '',
          '| | |',
          '|---|---|',
          ...
          '',
          'The shadow removes the `arrow` / `padding` / `shadow` props and applies shared defaults: ' +
            '`animate`, a 400ms open delay (`TOOLTIP_DELAY_PROPS`), `placement="bottom"` with corner ' +
            'fallbacks, and `portal`. A `string` `content` is wrapped in `Text size={1}`; a `hotkeys` ' +
            'array renders inline. The source itself notes: strongly prefer a short `string` `content` ' +
            'for i18n.',
          '',
          'Current puts the validation message where only a hover reveals it; Recommended shows the ' +
            'message inline and lets the tooltip add detail. The accessible-labeling pair contrasts a ' +
            'bare icon button with one that carries both a tooltip and an `aria-label`.',
        ].join('\n'),
      },
    },
  },

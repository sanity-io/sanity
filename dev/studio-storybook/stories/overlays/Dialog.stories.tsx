...
const meta: Meta<typeof Dialog> = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          'Dialog is what Studio throws up when it needs to stop an editor and ask something, ' +
            'delete this document, discard these changes, or hold a short focused form, and its ' +
            'width presets are coarser than the sentences they are asked to hold: a one-sentence ' +
            'confirm at the width every confirm dialog ships with sets its copy on a line nearly ' +
            '25 percent past comfortable reading length.',
          '',
          '|             |                                                                                                                                                                                                                                             |',
          '| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |',
          ...
          '',
          'The shadow’s job is to make every one of those look and behave the same: it takes ' +
            'the `@sanity/ui` primitive, pins the footer to at most a cancel and a confirm ' +
            'button, fills in translated default labels, and toggles body padding, so the header ' +
            'and body are composed while the frame handles the rest.',
          '',
          'The Studio `Dialog` wraps `@sanity/ui` `Dialog` and adds: a fixed footer layout ' +
            '(`footer.cancelButton` / `footer.confirmButton` plus an optional ' +
            '`footer.description`), localized fallback labels pulled from ' +
            '`common.dialog.cancel-button.text` / `common.dialog.confirm-button.text`, a ' +
            '`padding` boolean that toggles the default body padding, `animate=true` by default, ' +
            'and `bodyHeight` / `zOffset` passthrough. Confirm defaults to `tone="critical"`; ' +
            'cancel defaults to `mode="bleed"`.',
          '',
          'The two-variant pair reproduces the audit finding on the real component: Current is ' +
            'the stuck destructive confirm (spinner, no confirm button), Recommended resolves the ' +
            'wait with a bounded error, a live confirm, and a clear escape hatch.',
          '',
          '> **Why it matters:** width is a blunt instrument, and here it governs readability. ' +
            'The presets jump 320 to 640 to 960px, so a one-sentence confirm at the standard ' +
            'confirm width sets its copy on a line near 94 characters, past the 75-character ' +
            'reading-comfort ceiling with most of the field left empty. Cap the prose, a ' +
            'measure-limited container around 62 characters, not the dialog; reserve the larger ' +
            'widths for genuinely wide content like tables and side-by-side diffs. The full study ' +
            'is below.',
          '',
          '---',
          '',
          ...
          '`width` passes straight through to `@sanity/ui` `Dialog`, whose preset indexes ' +
            '`theme.sanity.container[width]` (px). The Studio wrapper sets no default of its own, ' +
            'so an omitted `width` inherits the primitive default of `0`. Body copy is `<Text ' +
            'size={1}>` (13px) inside the wrapper’s `padding={4}` box (space[4] = 20px each ' +
            'side), so the readable text field is `container[width] − 40px`. At 13px Inter (avg ' +
            'glyph advance ≈ 0.49em ≈ 6.37px) each preset yields:',
          '',
          ...
          '',
          'Principle: hold text-first dialogs to a 45 to 75ch measure. Confirmations, alerts ' +
            'and prompts are prose, and prose past ~75 characters per line lengthens the eye’s ' +
            'return-sweep and measurably slows reading. The presets are coarse, `0` lands at ' +
            '~44ch, then `1` jumps straight to ~94ch with nothing between, so a one-sentence ' +
            'confirm at `width={1}` sets its copy on a single ~600px line, ~25% past the comfort ' +
            'ceiling with most of the field left empty. The fix caps the _prose_, not the dialog: ' +
            'wrap body copy in a measure-limited container (`max-width` ≈ 62ch / ~396px) so the ' +
            'frame keeps room for header and footer while the text reads at a comfortable width. ' +
            'Reserve `width={2}` and up for genuinely wide content, tables, side-by-side diffs, ' +
            'media grids, never for sentences.',
          '',
          'Proposed scale (RFC): the jumps are too big at the bottom, where most dialogs live, ' +
            '320 to 640 to 960 in +320 steps, so there is no preset near the ~60 to 70ch text ' +
            'sweet spot. A finer low end, keeping `0` as the sensible default and letting ' +
            '`width={2}` land where `width={1}` is today, gives text-first dialogs a native ' +
            'preset and pushes the large sizes up where only tables and media need them. This ' +
            'renumbering is a `@sanity/ui` `theme.container` change, cross-cutting and breaking, ' +
            'an RFC for the design-system owners, not a story-layer edit; the measure-capped ' +
            'container above is the ship-now fix that needs no token change.',
          '',
          ...
          '',
          'Content type to width, measure governs text: confirmation / alert / prompt uses `0` ' +
            'or a measure-capped container (~62ch), never above ~75ch; short form uses today `1` ' +
            '(proposed `2`); multi-column form uses today `2` (proposed `3`); table or ' +
            'side-by-side diff uses today `2`–`3` (proposed `4`); media grid or canvas uses today ' +
            '`3`+ (proposed `5`).',
          '',
          'In production: `ConfirmDeleteDialog` and `ConfirmDiscardDialog` both ship at ' +
            '`width={1}`; the discard confirm’s entire body is the 65-character sentence "Are you ' +
            'sure you want to discard all changes since last published?" set on one 600px line (a ' +
            '~94ch field, unconstrained). `UnpublishVersionDialog` uses `width={0}` (~44ch) and ' +
            'is already comfortable. The Measure current/recommended pair reproduces the discard ' +
            'confirm and the measure-capped fix, each with a grayscale measure ruler under the ' +
            'copy.',
          '',
          'The page closes _in context_: the real Delete confirm for the book _Anna Karenina_, ' +
            'its reference check resolved to a concrete count (three documents point at it) ' +
            'before the editor commits, a live open, confirm, cancel flow.',
        ].join('\n'),
      },
    },
  },

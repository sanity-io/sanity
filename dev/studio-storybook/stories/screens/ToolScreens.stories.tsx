...
import {NoToolsScreen} from '../../../../packages/sanity/src/core/studio/screens/NoToolsScreen'
import {ToolNotFoundScreen} from '../../../../packages/sanity/src/core/studio/screens/ToolNotFoundScreen'
import {ScreenFrame} from '../../lib/screenFrame'

const meta: Meta = {
  title: 'Navbar & Shell/Screens/Tool Screens',
  parameters: {
    // No component prop type at meta level (two screens, one page); nothing to control.
    controls: {include: []},
    docs: {
      description: {
        component: [
...
export const ToolNotFound: Story = {
  ...
  render: () => (
    <ScreenFrame height={360}>
      <ToolNotFoundScreen toolName="structure" />
    </ScreenFrame>
  ),
}

export const NoTools: Story = {
  name: 'No configured tools',
  parameters: {
    docs: {
      description: {
        story:
...
export const SideBySide: Story = {
  ...
  render: () => (
    <Stack gap={4}>
      <Stack gap={3}>
        <Text size={0} muted>
          ToolNotFoundScreen - an editor with a stale link
        </Text>
        <ScreenFrame height={280}>
          <ToolNotFoundScreen toolName="structure" />
        </ScreenFrame>
      </Stack>
      <Stack gap={3}>
        <Text size={0} muted>
          NoToolsScreen - a developer mid-setup
        </Text>
        <ScreenFrame height={280}>
          <NoToolsScreen />
        </ScreenFrame>

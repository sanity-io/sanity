...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          'Tasks is the closest thing Studio has to workflow, and it is a binary To Do/Done ' +
            'checkbox: state a team can neither see nor enforce. Content itself has no named ' +
            'stages between draft and published.',
          '',
          '|        |                                                                                                                                                                                                                        |',
          '| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |',
          ...
          '',
          "The stories mount the **real** `TasksList` (the tasks sidebar's list body) over " +
            'fixture task documents. Assignee avatars resolve through the real `createUserStore` ' +
            'against a fixture-serving client; the document target chip resolves through the real ' +
            'preview pipeline (`useDocumentPreviewValues`) against a fixture universe. The status ' +
            'checkbox runs the real `useTaskOperations().edit()` against a stubbed addon-dataset ' +
            'client: toggling resolves, but the fixture list is static, so items do not move ' +
            'between groups.',
          '',
          'Harness notes: the three tasks singleton contexts (`TasksEnabledContext`, ' +
            '`TasksContext`, `TasksNavigationContext`) are value-seeded by `TasksStoryHarness` in ' +
            '`lib/mockCollabFixtures.tsx`, mirroring what `TasksProvider` and ' +
            '`TasksNavigationProvider` assemble at runtime, so empty states, which read the ' +
            'active sidebar tab from navigation context, render for real. Due dates are load-time ' +
            'offsets: the "due today" red treatment renders identically on any day.',
        ].join('\n'),
      },
    },
  },

...
Use italics for true emphasis (e.g., *in context*). Labels, such as “Why it matters”, appear bold only because they are structural, not emphasized content.
...
- **Pipe characters split cells:** Using `boolean | {reason}` in a table row will split it into more columns. Rephrase as "a boolean *or* a `{reason}` object".

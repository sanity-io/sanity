...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          "The list body inside the navbar's new-document button is deliberately dumb: no " +
            'schema reading, no permission checking, just four branches over the props it is ' +
            'handed, and the branches turn out to matter, because a permission problem and a ' +
          ...
          "`useNewDocumentOptions` (the caller's hook) tags every declared template with " +
            ...
            '`InsufficientPermissionsMessage`. The empty state this component owns ("No ' +
            'document types found") is reachable only when the caller\'s `options` array is ' +
            'itself empty, which happens when the studio declares no creatable types, or every ' +
            'declared type was filtered as deprecated one level up in `NewDocumentButton`, ' +
            'never as a side effect of permissions. `AllPermissionsDenied` and ' +
            '`NoDocumentTypesDeclared` below are the same message space and genuinely different ' +
            'inputs; they do not collapse.',
          '',
          '> **Why it matters:** this is the shape that broke elsewhere this week ' +
            ...
            'administrator or ask whoever owns the schema. This component keeps that ' +
            'distinction, because the deciding work of whether to remove an option was never ' +
            'pushed down into it in the first place. The caller keeps every option and lets ' +
            'this component render the honest, disabled truth.',
        ].join('\n'),
      },
    },
  },

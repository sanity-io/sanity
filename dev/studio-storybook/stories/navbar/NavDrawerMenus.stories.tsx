...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          'A preference menu that lives inside the navbar drawer is the kind of small, ' +
            'always-there control the shell owns rather than any document. The appearance ' +
            '(color scheme) menu reads the current scheme from context and offers System, ' +
            'Light, and Dark, with a checkmark on the active choice.',
          '',
          '| | |',
          '|---|---|',
          ...
          'Its sibling, the locale menu (`LocaleMenu.tsx`), belongs here too but needs a studio ' +
            'configured with multiple locales to show anything; that is a harness follow-up ' +
            '(see the navbar decomposition map).',
        ].join('\n'),
      },
    },
  },

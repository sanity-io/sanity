...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          'CircularProgress and LinearProgress are two correct determinate progress primitives ' +
            'that Studio already ships but rarely reaches for. The audit found panes going blank ' +
            'on load with no skeleton, not because these components are broken, but because ' +
            'nothing calls them there.',
          '',
          '|          |                                                                                                                                                                                                                                                                            |',
          '| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |',
          ...
          '',
          '`CircularProgress` clamps with `Math.min(Math.max(value, 0), 100)`; `LinearProgress` ' +
            'translates by `value - 100%` and is clipped by an `overflow: clip` root, so 120 and ' +
            '-20 both render as full / empty rather than spilling. The clamping story shows both ' +
            'extremes.',
          '',
          '> **Why it matters:** storied here so the fix has a component to reach for, rather ' +
            'than needing a new one built.',
        ].join('\n'),
      },
    },
  },

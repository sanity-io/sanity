    ...
    docs: {
      description: {
        component: [
          ...
          '',
          'Hand it any `@sanity/icons` glyph and a `tone` key and it renders the icon in the ' +
            'theme colour for that status. Wraps any `@sanity/icons` glyph and tints it with the ' +
            'theme `--card-badge-<tone>-icon-color` variable. It is the shared tinting primitive ' +
            'behind status glyphs across Releases, Variants, and the perspective menu. The ' +
            'gotcha: ToneIcon only writes the `--card-icon-color` custom property, the rule that ' +
            'reads it lives on `@sanity/ui` `<Text>`, so it must be nested in a `<Text>` (as ' +
            'every real call site does) or it renders in the inherited text colour.',
          '',
          'Addressed for `similarity` looks like the `Current` vs `Recommended` pair below: ' +
            'pairing each tone with a distinct icon shape and a text label so the status survives ' +
            'a grayscale render.',
          '',
          '> **Why it matters:** ToneIcon writes a CSS custom property; a surrounding text ' +
            'element applies the colour. Drop it anywhere that is not inside that element and the ' +
            'tone is silently ignored: the icon renders in the inherited text colour, exactly as ' +
            'the wrap-gotcha story shows.',
          '',
          'The last story shows it in context: the validation summary for the "Anna Karenina" ' +
            'draft, each field issue led by a tone-coloured status glyph.',
        ].join('\n'),
      },
    },
  },

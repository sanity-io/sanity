...
const meta: Meta<typeof BetaBadge> = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          "BetaBadge is Studio's marker for a feature that has shipped but is not yet finished, " +
            'a Content Release, a new inspector, an experiment. It is deliberately impossible to ' +
            'recolor.',
          '',
          '| | |',
          '|---|---|',
          ...
          '',
          'Every not-yet-stable feature in Studio wears the same marker, so the signal reads ' +
            'the same everywhere an editor meets it. The `children` default is "Beta", but any ' +
            'short label works ("Alpha", "New", "Experimental"); `fontSize` is the only real ' +
            'dial. The sweeps below show the default, alternate labels, and the font-size scale.',
          '',
          '> **Why it matters:** tone and mode are deliberately not exposed, so it cannot be ' +
            'recolored. That uniform primary tone is the point: an experimental marker only works ' +
            'as a signal if it looks identical everywhere, so the badge trades flexibility for ' +

...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          'An editor who sees the bare skeleton has no way to know whether to wait two seconds ' +
            'or two minutes, and an editor who sees the error card has no way to know whether ' +
            'Retry will work, is pointless, or needs someone else’s attention: three remedies, ' +
            'one screen.',
          '',
          '| | |',
          '|---|---|',
          ...
          'Every story below runs the REAL `useVideoPlaybackInfo` hook against the REAL ' +
            '`VideoPreview` dispatch; nothing is stubbed. What is controlled is the one REST ' +
            "call the hook makes (`client.request({uri: '.../playback-info'})`), using the mock " +
            "client's own `requests`/`requestCallback` seam " +
            '(`packages/sanity/test/mocks/mockSanityClient.ts`) rather than a fabricated ' +
            'backend, the same mechanism the rest of this storybook already trusts for every ' +
            'other client-backed story.',
          '',
          ...
          '"Still processing" (Media Library asset `state: "processing"`, polled for up to 2 ' +
            ...
            '(`startWith(loadingState)` fires once, nothing re-emits mid-poll), so it is the ' +
            'bare `<VideoSkeleton />`, identical to the very first instant of loading. "Failed ' +
            'to process" (`doc.state` is anything other than `"ready"`/`"processing"`), ' +
            '"deleted" (the asset document is not found at all, see the `DeletedOrMissing` ' +
            'story, whose own error message claims to be retrying and is not), and a genuine ' +
            'network failure fetching `/playback-info` (any non-404 rejection) all land in the ' +
            'SAME branch, `playbackInfoState.error`, with the SAME critical-toned card and Retry ' +
            'button, distinguished only by whatever raw `Error.message` string happens to have ' +
            "been thrown. None of the three has a curated, translated message; `VideoSkeleton`'s " +
            'one translated string (`video-error.description`) is reached only when ' +
            '`error.message` is falsy, which none of the errors this file constructs are (see ' +
            '`NoMessage` below for the one case that does reach it).',
          '',
          '</details>',
          '',
          ...
          'The playback-info response carries `thumbnail`, `animated` and `storyboard` URLs ' +
            ...
            '"before playback" surface `VideoPreview`\'s own chain renders is ' +
            "`VideoActionsMenu`'s `React.lazy` `<Suspense fallback={<VideoSkeleton " +
            'aspectRatio={...} />}>` while the `VideoPlayer` chunk itself downloads, a THIRD ' +
            'situation sharing the bare-skeleton appearance from the previous finding, this time ' +
            'for "the code to play the video has not arrived yet" rather than "the video itself ' +
            'has not arrived yet". Whatever a viewer sees before a frame decodes belongs ' +
            "entirely to `@mux/mux-player-react`'s own internal behaviour (it does receive " +
            '`tokens.thumbnail`/`tokens.storyboard`), which is outside this codebase and not ' +
            'traced further here.',
          '',
          '</details>',
          '',
          ...
          '`VideoPlaybackInfo.duration` (seconds) is set from every resolved response and never ' +
            'referenced anywhere else in `VideoInput/` (grepped: zero other hits). ' +
            '`ActionsMenu` (the shared menu this component reuses from the file-input family) ' +
            'offers upload/browse/download/copy-url/open-in-source/reset and nothing else: no ' +
            'length, no dimensions. Size is not even modelled: unlike image assets, ' +
            '`VideoPlaybackInfo` has no size field at all. A person decides whether to open a ' +
            'video with strictly less information than they get for an image.',
          '',
          '</details>',
          '',
          ...
          '</details>',
          '',
          '> **Why it matters:** an editor who sees the bare skeleton has no way to know whether ' +
            'to wait two seconds or two minutes, and an editor who sees the error card has no ' +
            'way to know whether Retry will work (a real network blip), is pointless (the asset ' +
            "is gone), or needs someone else's attention (processing failed at the source): " +

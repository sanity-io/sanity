      ...
      description: {
        component: [
          ...
          'This is where the actual work happens: the document editor itself, the whole ' +
            ...
            'If you want to understand how editing works in this product, you start here.',
          '',
          'The real editor is mounted here, not a header or status bar in isolation, over a real ' +
            'document store. The pair machinery runs for real, snapshots served from fixtures, a ' +
            'listener held open on a mock welcome event, grants come from a fixture access list, ' +
            'and edits type into the real local-first patch pipeline, nothing persists, mutations ' +
            "land in the mock client's own transaction log. The document header, context menu, " +
            'view tabs, the form, and the status bar, publish button, action menu, status line, ' +
            'are all live.',
          '',
          'The events feed is served empty, a shaped response, so the pane renders its real ' +
            '"no events" state rather than a network error. Revision and event list contents are ' +
            'out of scope for this harness tier.',
          '',
          '> **Why it matters:** lifecycle state, draft, published, edited, is carried by ' +
            'color-only status dots of identical shape and size, and a version or variant chip can ' +
            'surface before any version exists. This is the core surface every editor lives in, so ' +
            'those two findings do not touch one screen. They touch every document in the ' +
            'product.',
        ].join('\n'),
      },
    },

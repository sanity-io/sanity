...
const meta: Meta = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          'Annotations, decorators, styles, and list items customise the text of a Portable Text ' +
            'field rather than the objects embedded in it, and all four are schema-level only: ' +
            'none has a workspace-config equivalent, because there is nothing studio-wide to say ' +
            'about a decorator that only exists because a particular field declared it.',
          '',
          '| | |',
          '|---|---|',
          ...
          'Note the inconsistency in that block, because it is real and it will cost you five ' +
            'minutes. Styles, lists and decorators take a singular `component`. Annotations take a ' +
            '`components` object with an `annotation` key. The reason is that an annotation is a ' +
            'full object type with fields, so it carries the same `components` bag every object ' +
          ...
          '',
          'Each story turns on exactly one of the four against an identical document, so the diff ' +
            'is always attributable.',
          '',
          '> **Why it matters:** for a block object you decorate by wrapping the default render. ' +
            'For an annotation you generally cannot, because an annotation is a mark on a span ' +
            'rather than a container around one, and inserting an element around the default would ' +
            'put a boundary inside a run of editable text. The move instead is to call the default ' +
            'renderer with a modified prop, passing back a decorated text element rather than ' +
            'wrapping the whole thing. Same principle, inverted mechanics.',
        ].join('\n'),
      },
    },
  },

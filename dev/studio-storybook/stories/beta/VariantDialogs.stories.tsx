...
const meta: Meta = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          'A variant is a rule rather than a piece of content, a set of conditions deciding ' +
            'which readers see which version of a document, and that makes changing one unlike ' +
            'editing anything else in the studio.',
          '',
          '|          |                                                        |',
          '| -------- | ------------------------------------------------------ |',
          ...
          '',
          'Versioning/Variants already stories the create dialog and the tool. These are the ' +
            'two the board flagged as gaps: edit its definition, or delete it.',
          '',
          '`EditVariantDialog` reuses the same `VariantDialog` form the create flow uses, ' +
            'seeded from the existing definition, which is the right call: the thing being edited ' +
            'is the same shape as the thing that was created, and a separate edit form would be ' +
            'an opportunity for the two to drift.',
          '',
          '`DeleteVariantDialog` is the sharper one. Deleting a variant does not delete the ' +
            'documents written against it; it removes the rule that routed readers to them. The ' +
            'dialog names the variant it is about to remove, because "delete variant?" with no ' +
            'name is exactly the confirmation people click through.',
          '',
          '> **Why it matters:** editing a variant changes every document it already applies ' +
            'to, immediately, with no draft state to preview the effect first. Treat an edit like ' +
            'a rule change, not a content edit.',
        ].join('\n'),
      },
    },
  },

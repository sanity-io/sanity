...
const meta: Meta = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          'Linking a document to Canvas never redirects silently: this flow runs a preflight, ' +
            'and when the mapping into Canvas would alter content, it shows the diff and asks ' +
            'first.',
          '',
          '|          |                                                                                                                                                                                                                                                   |',
          '| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |',
          ...
          '',
          'That confirmation body is the substance worth studying here. Linking a document to ' +
            "Canvas runs a preflight that maps the Studio document into Canvas's model; when that " +
            'mapping would change content, the dialog shows the diff and asks the author to ' +
            'confirm before redirecting. `useLinkToCanvas` is deep and backend-bound (it resolves ' +
            'the Studio appId, POSTs a preflight, then resolves the organization redirect), so ' +
            'the _diff and redirecting states require a live Canvas backend_ and are not ' +
            'reachable offline.',
          '',
          'What this file stories instead: the confirmation diff body (`LinkToCanvasDiff`) ' +
            'rendered directly with fixture documents, a real `@sanity/diff` comparison through ' +
            'the real `ChangeList`, which is the substantive content of the dialog, plus the ' +
            'dialog’s two deterministic offline states (missing document id, Studio app not ' +
            'found), driven by a stub AppId cache. The linked-document banner ' +
            '(`CanvasLinkedBanner`) is a separate surface, storied under Document Banners/In a ' +
            'live pane → "Canvas linked", not duplicated here.',
          '',
          '> **Why it matters:** the whole flow is opt-in, `apps.canvas.enabled` defaults off, ' +
            'so none of this runs, no dialog, no redirect, no banner, unless a workspace turns ' +
            'Canvas on.',
        ].join('\n'),
      },
    },

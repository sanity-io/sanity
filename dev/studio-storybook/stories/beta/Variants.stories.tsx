...
const meta: Meta = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          'One document, many audiences: Content Variants let a single piece carry per-audience, ' +
            'per-market, or per-locale copies, each targeted by conditions and shown to the right ' +
            "reader automatically. This is Sanity's headline bet on personalised content, and its " +
            'value is still under-told.',
          '',
          '| | |',
          '|---|---|',
          ...
          '',
          'The idea is simple to say and deep to build: write once, define who each variant is for, ' +
            'and let the perspective router serve the matching copy. If you want to understand where ' +
            'Studio is heading on targeted content, start here.',
          '',
          'Content Variants let one document group carry per-audience/per-locale variant copies, ' +
            'targeted by `conditions` (audience, market, locale, plan, and so on) with a `priority` ' +
            'order. A variant *definition* is a `system.variant` document at `_.variants.<id>`; the ' +
            '*content* lives in ordinary version documents whose `_system.variant` points back at ' +
            'the definition. The surfaces here run the real store, hooks and table code against a ' +
            'fixture client (list and counts queries) and a fixture preview store (membership ' +
            'id-set); see `lib/variantsFixtures.ts`.',
          '',
          'The `Overview` and `Detail` stories mount the whole `VariantsTool` on a stateful router: ' +
            'clicking a variant row opens the real detail view (grouped document table, release ' +
            'bundle chips resolved from the seeded releases store, live validation on the title-less ' +
            'fixture document).',
          '',
          '> **Why it matters:** keep the two halves straight. A variant definition is a system.' +
            'variant document that describes an audience; the variant content lives in ordinary ' +
            'version documents that point back at that definition. The definition is the rule, the ' +
            'version is the copy, conflate them and the model stops making sense.',
        ].join('\n'),
      },
    },
  },

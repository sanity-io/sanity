...
const meta: Meta = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          "Divergences is how Studio makes reconciling two people's diverging edits legible, " +
            'instead of a merge conflict in the dark.',
          '',
          '|          |                                                                                                                                                                                                                                                                               |',
          '| -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |',
          ...
          '',
          'A pencil beside each field that moved, and a side-by-side of what changed upstream ' +
            'so the editor can choose. If you are building on Advanced Version Control, this is ' +
            'the surface authors will actually feel.',
          '',
          'A divergence is detected per document node by comparing three snapshots: the ' +
            'upstream at the fork point, the upstream head, and the subject head. The editor sees ' +
            'a pencil indicator beside each diverged field; focusing one opens the resolution ' +
            'overlay showing the upstream base-to-head diff (computed live by `@sanity/diff` and ' +
            "rendered by the field's real diff component) with two resolutions: Ignore (mark " +
            'resolved at the current upstream revision) or Copy from base (take the upstream ' +
            'value).',
          '',
          'These stories enter at the navigator seam (`lib/divergenceFixtures.ts`): the ' +
            ...
            'fetches its data through the real paths, `getDocumentAtRevision` against the history ' +
            'endpoint (fork snapshot) and `documentStore.pair.editState` (upstream head), so the ' +
            'diff you see is genuinely computed from the two fixture documents.',
          '',
          '> **Why it matters:** every field offers two resolutions and they are not symmetric. ' +
            ...
            'changing your value; Copy from base overwrites your value with the upstream one. An ' +
            'author who reaches for the wrong one loses their edit, so the label and the diff ' +
            'have to make the consequence obvious before the click.',
        ].join('\n'),
      },
    },
  },

...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          'Something you build only helps editors if it reaches them, and this is how Studio does ' +
            'that without sending an email: a floating teaser, a full reader, and a Help-menu entry ' +
            'that surface a shipped feature to the people already at work.',
          '',
          '| | |',
          '|---|---|',
          ...
          '| Patterns | `whats-new` |',
          '',
          'Rather than hoping editors read a changelog, the announcements surface meets them where ' +
            'they work, and because it is chrome painted over the shell, it has to earn that attention ' +
            'without ever getting between the editor and their document.',
          '',
          'The three pieces are prop-driven and render offline. `StudioAnnouncementsCard` is the ' +
            'bottom-left floating teaser; `StudioAnnouncementsDialog` is the full reader (one entry ' +
            'per unseen announcement, Portable-Text body via the shared upsell serializer, sticky date ' +
            'header); `StudioAnnouncementsMenuItem` is the Help-menu re-entry point. The provider that ' +
            'fetches the feed and tracks "seen" state is not mounted, fixtures stand in for its output.',
          '',
          '> **Why it matters:** this surface lives or dies by whether it can be dismissed. A ' +
            ...
            "bleed close button and the dialog's click-outside both honor that; if you extend this " +
            'surface, keep the escape hatch.',
        ].join('\n'),
      },
    },
  },

...
const meta: Meta = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          'These dialogs make the scheduled-publish hand-off honest. Scheduling a document to ' +
            'publish itself is a small feature with a big trust requirement: the author is ' +
            'handing the publish button to a clock.',
          '',
          '|          |                                                                                                                                                                                                                                                                                   |',
          '| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |',
          ...
          '',
          'Spelling out exactly what happens and when means nobody is surprised by content ' +
            'going live while they sleep. Under the hood it is the full releases machinery, ' +
            'deliberately wearing a simpler hat.',
          '',
          'A scheduled draft is a single-document `scheduled` release: the edited version ' +
            'document is parked in a release whose `intendedPublishAt` fires the publish. Both ' +
            'dialogs run the real hooks, `useScheduledDraftDocument` resolves the version ' +
            'document (and its live `Preview`) through the release-bundle seam, and ' +
            '`useScheduleDraftOperations` provides the confirm handlers, against a fixture ' +
            'preview store (a `partOfRelease` id-set override) and a seeded releases store. See ' +
            '`lib/scheduledDraftFixtures.ts`.',
          '',
          'The confirm handlers are the real operations store running against the mock client; ' +
            'in a static story nothing invokes them (autodocs never clicks), so the dialogs ' +
            'render their upcoming-schedule state cleanly. The override banner and navbar menu ' +
            'item are gated surfaces too, the banner is storied under Document Banners/In a live ' +
            'pane → "Scheduled draft override", not duplicated here.',
          '',

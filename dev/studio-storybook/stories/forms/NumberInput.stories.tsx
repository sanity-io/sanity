...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          'NumberInput closes a real gap: a plain `type="number"` input will happily change its ' +
            'value on an accidental scroll, a classic way to corrupt data without noticing. It ' +
            'also picks the right mobile keyboard along the way.',
          '',
          '|           |                                                                                                                                                                                                                                                                                                                 |',
          '| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |',
          ...
          'It looks like a plain text box, but it is quietly schema-aware. From the field’s own ' +
            '`min` / `integer` / `precision` rules it picks the right mobile keyboard: a number ' +
            'pad for integers, a decimal pad for prices, plain text when nothing constrains it, ' +
            'so an editor on a phone gets sensible keys. And it installs a wheel guard, so an ' +
            'accidental scroll over the field never silently nudges the value up or down.',
          '',
          'Under the hood it wraps `@sanity/ui`’s `TextInput` at `type="number"`, mounted for ' +
            'real inside the real `FormField` chrome via `fieldTestHarness`. It inherits the same ' +
          ...
          '> **Why it matters:** a `type="number"` input will happily change its value when you ' +
            'scroll over it, a classic way to corrupt data without noticing. NumberInput installs ' +
            'a wheel guard so that cannot happen; if you ever rebuild this field, keep that ' +
            'guard.',
          '',
          'The page closes **in context**: the numeric fields at work, a review of _Anna ' +
            'Karenina_ with its rating and price side by side in one live form.',
        ].join('\n'),
      },
    },
  },

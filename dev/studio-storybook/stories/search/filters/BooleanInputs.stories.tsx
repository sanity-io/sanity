...
import {SearchFilterBooleanInput} from '../../../../../packages/sanity/src/core/studio/components/navbar/search/components/filters/filter/inputs/boolean/Boolean'
import {
  FilterInputFrame,
  OperatorInputStory,
  WithFilterProviders,
...
const meta: Meta = {
  ...
  parameters: {
    docs: {
      description: {
        component: [
          'A two-option select stands in for the switch behind every true/false filter, and it ' +
            'is also the shape of a genuinely different kind of operator: one that carries no ' +
            'value at all and so has no input to show.',
          '',
          '| | |',
          '|---|---|',
          ...
          '| Patterns | `filters` |',
          '',
          'This is the value control for a boolean-equals operator; the valueless case, ' +
            '`defined` and `notDefined`, is the last story on this page.',
          '',
          '> **Why it matters:** the boolean input is a controlled select, not an uncontrolled ' +
            'text box, so it has no empty state to speak of, it always displays either True or ' +
            'False. That makes it a useful contrast with the string and number families: those ' +
            'need to distinguish nothing typed yet from a real value, this one cannot, because a ' +
            'two-option select is always showing one of its two options.',
        ].join('\n'),
      },
    },
  },
...
export const BooleanUnset: Story = {
  ...
  render: () => (
    <FilterInputFrame>
      <OperatorInputStory input={SearchFilterBooleanInput} fieldPath="featured" />
    </FilterInputFrame>
  ),
}
...
export const BooleanTrue: Story = {
  ...
  render: () => (
    <FilterInputFrame>
      <OperatorInputStory
        input={SearchFilterBooleanInput}
        fieldPath="featured"
...
export const BooleanFullscreen: Story = {
  name: 'Boolean, full-screen sizing',
  parameters: {
    docs: {
      description: {
        story:
...
export const ValuelessOperators: Story = {
  name: 'Valueless operators: defined / not defined',
  parameters: {
    docs: {
      description: {
        story:
      ...
      <Stack gap={3} style={{maxWidth: 420}}>
        <Card padding={3} radius={2} tone="transparent" border>
          <Stack gap={3}>
            <Text size={1} weight="medium">
              publishedAt not empty
            </Text>
            <Text muted size={1}>
              No value card renders below this line. Compare the boolean stories above, each of
              which adds a <Code size={1}>{'<Card borderTop>'}</Code> holding the select - here that
              card is never mounted, because <Code size={1}>operator.inputComponent</Code> is{' '}
              <Code size={1}>undefined</Code> for both <Code size={1}>defined</Code> and{' '}
              <Code size={1}>notDefined</Code>.
            </Text>
          </Stack>
        </Card>
        <Card padding={2} radius={2} tone="transparent">
          ...
          <Code size={0} style={{whiteSpace: 'pre-wrap'}}>
            {'defined(publishedAt)   //  defined\n!defined(publishedAt)  //  notDefined'}
          </Code>
        </Card>
      </Stack>
    </FilterInputFrame>

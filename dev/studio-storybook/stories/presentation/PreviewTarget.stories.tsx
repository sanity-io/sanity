...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          "This page is the load target for Presentation's iframe stories. Rather than stand up " +
            'a second server, it serves itself: same origin, no extra process, ships with the ' +
            'static build for free.',
          '',
          '| | |',
          '|---|---|',
          ...
          '| Tier | harness |',
          '',
          '`Preview` opens a `@sanity/comlink` channel to whatever loads in its iframe, so ' +
            'storying it needs a front end. A real Next.js app would work and would become a ' +
            'second process that has to be running for these stories to pass, in CI and in the ' +
            'portable tarball. Storybook already serves every story at `iframe.html?id=…` on the ' +
            'same origin, so the target is just another story: no extra server, and it ships with ' +
            'the static build for free.',
          '',
          '> **Why it matters:** the handshake is real. `Preview` creates the controller ' +
            'channel and this page creates the matching node, using the actual protocol, so the ' +
            'Presentation machine genuinely leaves its loading state. But visual editing itself ' +
            'is not: a production front end wraps content in stega-encoded values and ' +
            'click-to-edit overlays, and this page renders a plain document that only answers the ' +
            'handshake. The connection is real; the editing loop is not, and the Presentation ' +
            'stories say so rather than implying otherwise.',
        ].join('\n'),
      },
    },
  },

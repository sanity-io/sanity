...
- **Git history as data.** One `gitCommit` document per main-branch commit
  ...
  main re-upserts the last 200 commits — deterministic ids + createOrReplace
  make that stateless and self-healing, and a gap larger than the window
  needs one `backfill` dispatch. Because documents are replaced whole, a run
  never writes what it could not collect: a GitHub or npm API error aborts
  the run before anything is written (failing loudly — the workflow alerts
  Slack, and re-running is always safe), and tags are written only on
  npm-collecting runs.

  npm enrichment: tags also carry `publishedAt`, `distTags` and
  `weeklyDownloads` under `npm`, collected on tag pushes, the daily cron,
  and dispatches (`--npm`) — releases are far rarer than pushes, and the
  cron is the floor because dist-tags re-point and download counts roll
  without commits.
  ...
  documents carry `git.commit`, a weak reference to their `gitCommit` doc
  (written by perf/bench's storeShape.ts, backfilled once via the
  `patch_bench_run_refs` dispatch input; it dangles for PR-branch runs by
  design, and `git.sha` stays the source of truth). And each `gitCommit`
  records `testStudioUrl`, the immutable Vercel deploy of dev/test-studio
  built at that commit — collected (not built) from GitHub deployment
  statuses, landing one sync late by design since the Vercel build outlives
  the sync run.

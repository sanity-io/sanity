...
export default defineCliConfig({
  ...
  reactCompiler: {
    ...
    // pre-compiled with `tsdown`, and so we filter by just workspace sources (substring filters;
    // the `monorepo` condition resolves them under `src/` during `sanity dev`)
    sources: ['/sanity/src/', '/@sanity/vision/src/'],
  },

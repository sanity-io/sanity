...
- **`pnpm dev` needs a built `sanity` package.** On a fresh VM `sanity dev` fails with `Cannot find module '.../node_modules/sanity/lib/cli.js'` because the CLI config loader comes from the built package. `pnpm exec turbo run build --filter=sanity` (about 15 s with tsdown, builds the workspace deps too) is enough. `packages/sanity/test/exports.test.ts` also needs those dependency builds (`@sanity/types/lib`, `@sanity/validation/lib`), while the jsdom suites under `packages/sanity/src` run from sources. Then revert the `package.json` churn described below.

...
- **`tsc` at the repo root is TypeScript 7; the TS 6 API comes from an aliased package.** The root `typescript` dependency is aliased to `@typescript/typescript6` (TypeScript 7 ships no programmatic API, and tools like typedoc and ts-morph need the TS 6 API), while the root `typescript7` dependency (`npm:typescript@^7`) provides the native TS 7 `tsc` binary used by vitest's typecheck (`*.test-d.*` tests) and by tsdown `dts: {tsgo: true}` via each package's `typescript` catalog dependency. CI type checking of application code is owned by oxlint (`options.typeCheck`). The old symlink workaround for a missing root `tsc` is no longer needed.

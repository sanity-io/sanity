...
- **`pnpm build` rewrites `packages/sanity/package.json`.** It drops `@sanity/sdk` and `zustand` from the `inlinedDependencies` block. This reproduces on a clean `main`, so it is not caused by your change — `git checkout -- packages/sanity/package.json` before staging, and never include it in a PR that isn't about bundling.

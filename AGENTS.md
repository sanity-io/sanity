...
| **Type Check**   | `pnpm check:types`    | TypeScript 7 (native `tsc`) + turbo                                                                                                                              |
...
- **`tsc` at the repo root is TypeScript 7; the TS 6 API comes from an aliased package.** The root `typescript` dependency is aliased to `@typescript/typescript6` (TypeScript 7 ships no programmatic API, and tools like typedoc, typescript-eslint, and `@sanity/pkg-utils` need the TS 6 API), while the root `typescript7` dependency (`npm:typescript@^7`) provides the actual TS 7 `tsc` binary used by `check:types` scripts and vitest's typecheck (`*.test-d.*` tests). Packages built with `@sanity/pkg-utils` intentionally do NOT declare `typescript` — pnpm auto-installs the TS 6 peer for pkg-utils in those contexts; adding `typescript` (v7) as a dependency there breaks their builds with `Cannot read properties of undefined (reading 'fileExists')`. The old symlink workaround for a missing root `tsc` is no longer needed.

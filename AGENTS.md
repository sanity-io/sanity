...
- **`pnpm build` rewrites `packages/sanity/package.json`.** The build regenerates the `inlinedDependencies` field, so a build leaves that file modified even when you changed nothing in it. Check `git status` after building and `git checkout -- packages/sanity/package.json` before committing, unless the change is genuinely yours.
- **A release seeded via the API is invisible to an already-open studio.** The release list is loaded once, so a `?perspective=s:<releaseId>` URL for a release created after page load resolves to "Not in the … release". Do a full page reload (or navigate to `/test/releases` and reload) after seeding, then open the document from the release detail view.

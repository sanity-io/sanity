...
**Firefox shards still need Chromium.** `e2e/globalSetup.ts` always calls `chromium.launch()` (Playwright `projects[0]` is Chromium) even when the job is `--project firefox`, and since Playwright 1.49 headless Chromium is its own `chromium-headless-shell` download. The browser install on every shard must therefore include Chromium, not just `matrix.project`: otherwise a `~/.cache/ms-playwright` miss leaves Firefox shards failing in global setup with `Executable doesn't exist .../chromium_headless_shell-*/chrome-headless-shell`, before any spec runs (the Playwright report then shows 0 test failures). See `.github/workflows/e2e.yml`.

Treat that cache miss as the normal case. The repo keeps hundreds of active caches, so the ~400 MB `1.x.x-playwright-browsers-chromium-firefox` entry is evicted often, and the `install` job's `actions/cache@v6` only writes on a miss — when it hits an entry that is evicted moments later, every shard in that run misses and nothing re-saves it. Because a cache hit hides this bug, a rerun can go green with no code change; do not read that as a flaky assertion.

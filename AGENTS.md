...
- To compare re-renders against `main`, script the same interaction with Playwright, call `profile start` / `profile stop` / `profile export <file>` from the script, and run it once per version. Swap versions with `git checkout main -- <dir>` and back with `git checkout HEAD -- <dir>`, then delete files that only exist on `main` (the swap stages them). Restart `sanity dev` after swapping back: its module graph keeps imports of the deleted files and the studio stops loading. Per-component render counts in the exported JSON come from `dataForRoots[0].commitData[].fiberActualDurations` (fiber ids) mapped through `snapshots` (display names). Profiling sessions compile many lazy chunks, so run them with `unstable_bundledDev: false` (see the bundledDev gotcha below) and revert that before committing.
- Counting global search requests: filter on the `tag=sanity.studio.search.global` query parameter, and keep only queries containing the `findability-source: global` comment. The `groq2024` strategy first sends a reference-resolution query with the same tag and no `findability` comments, whenever a query term meets a type whose preview follows references.

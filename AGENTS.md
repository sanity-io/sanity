...
- Playwright writes the `error-context.md` page snapshot after the test-scoped fixtures have torn down, and `_testContext` deletes the drafts a spec seeded through `sanityClient` at that point. For API-seeded documents the snapshot therefore shows "This document has been deleted" and read-only fields whatever the test actually saw; judge the Studio state from the failure video or your own assertions instead.
- To exercise the Studio embedded in a cross-site iframe (the dashboard topology), fulfill the top-level document with `page.route` and seed the session token from a same-origin bootstrap document that writes `__studio_auth_token_<projectId>` to `localStorage` before navigating to the Studio: Firefox partitions third-party `localStorage` by top-level site, so the `storageState` token is not visible inside the iframe. Playwright's Firefox also runs with `dom.security.https_first=false`, so a plain `http://` stand-in top-level works. `e2e/tests/inputs/imageHotspotEmbeddedFirefox.spec.ts` has the pattern.

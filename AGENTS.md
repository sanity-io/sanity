...
A second bug has the same symptom (ui5 surfaces such as comments rendered for the OS appearance while the rest of the studio follows the Studio appearance) but a different cause, so tell them apart before fixing either. Native `light-dark()` follows the document's `color-scheme`, and the ui5 reset only sets `:root { color-scheme: light dark }`, which lets the OS decide. `GlobalStyle` therefore pins `html:root { color-scheme }` to the Studio appearance (`light` / `dark`, `light dark` for `system`, covered by `GlobalStyle.browser.test.tsx`) — purely render-driven, no effects or pre-mount DOM writes — so any studio that renders the global styles is covered, while hosts that disable `unstable_globalStyles` have to set `color-scheme` on `<html>` themselves. When contrast is wrong, `--lightningcss-light` / `--lightningcss-dark` custom properties in the built CSS mean the polyfill above; a document `color-scheme` that disagrees with the Studio appearance means this one.

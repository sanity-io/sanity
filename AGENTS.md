...
- **`pnpm dev` binds IPv6 loopback only.** Vite serves on `::1`, so `http://localhost:3333` works but `http://127.0.0.1:3333` refuses the connection. To reach the studio on another interface, run `npx sanity dev --host 0.0.0.0 --port 3333` from `dev/test-studio` instead.
- **CORS problems do not reproduce from a loopback origin.** The API treats `localhost` and `127.0.0.1` as always-allowed, on both the project host (`<projectId>.api.sanity.io`) and the global host (`api.sanity.io`). Requests that a real deployment would see blocked therefore succeed from the local dev studio. Two consequences:
  - To confirm what an origin is actually allowed, ask the API directly rather than inferring from the browser: `curl -s -H "Origin: https://example.com" https://<projectId>.api.sanity.io/vX/check/cors`. Compare against the same path on the bare `api.sanity.io` — the global host has no project context and always answers `allowed: false`.
  - A faithful browser repro needs a non-loopback origin that is _also_ in the project's CORS allowlist. `STUDIO_AUTH_TOKEN` cannot read or edit the allowlist for `ppsg7ml5` (`sanity.project.cors/read` is denied), so verify these changes with `curl` plus unit tests, and use the studio only to confirm which host a request is sent to (visible in the devtools Network panel).

...
interface RetryingClickOptions {
  ...
  /**
   * Pass `testInfo` (from the test callback's second argument, or `test.info()`)
   * to attach portal-overlay screenshots to the Playwright HTML report.
   * Without this, diagnostics are only written to console.log (visible in CI job logs).
   */
  testInfo?: TestInfo
}

async function attachScreenshot(page: Page, name: string, testInfo?: TestInfo): Promise<void> {
  if (!testInfo) return
  try {
    const screenshot = await page.screenshot()
    await testInfo.attach(name, {body: screenshot, contentType: 'image/png'})
  } catch {
    // Screenshot or attachment failed, continue
  }
}
 ...
 *    bounding boxes) and optionally attaches a screenshot to the test report.
...
export async function retryingClick(
  ...
  const {maxRetries = 3, perAttemptTimeout = 5_000, retryDelay = 1_000, testInfo} = options ?? {}
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (diagnostics && diagnostics.visibleChildren.length > 0) {
        ...
        await attachScreenshot(page, `portal-overlay-attempt-${attempt}`, testInfo)
      } else {
        console.log(
...
export async function retryingClickUntilVisible(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    if (attempt === maxRetries) {
      ...
      await attachScreenshot(page, `portal-overlay-final-failure`, testInfo)

...
test.describe('maximized document', () => {
  test.beforeEach(async ({page, createDraftDocument}) => {
    ...
    await expect(
      page.getByTestId('sanity-form-field-title').getByTestId('string-input'),
    ).toBeVisible()

    await page
      .getByTestId('sanity-form-field-title')
  ...
  test('pane focuses on the middle pane, will close references to the right', async ({page}) => {
    ...
    await expect(
      page.getByTestId('sanity-form-field-title').getByTestId('string-input').nth(1),
    ).toBeVisible()

...
export function expectManifestSchemaConversion(schema: Schema, schemaDescriptor: Descriptor) {
  ...
  for (const convertedObjectValue of convertedObjectValues) {
    ...
    expect(convertedObjectValue.name).toStrictEqual(schemaObjectValue?.name)
    expect(convertedObjectValue.type).toStrictEqual(schemaObjectValue?.type)

    const titles = extractTitleMap(convertedObjectValue.typeDef)
    const stype = normalizeSchemaDescriptorTypeDef(cloneDeep(schemaObjectValue?.typeDef), titles)
...
function extractTitleMap(data: unknown) {
  ...
  function extract(obj: any, path: string) {
    ...
    if (obj.title && startCase(obj.name) !== obj.title) {
...
function normalizeSchemaDescriptorTypeDef(
  ...
  if (!isEncodableObject(obj)) return obj
...
function isObject(val: unknown): val is any {

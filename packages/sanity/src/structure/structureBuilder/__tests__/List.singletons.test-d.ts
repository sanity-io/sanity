...
test('S.list().singletons() takes no arguments, narrowing identically', () => {
  const list = S.list().singletons()
  expectTypeOf(list).toEqualTypeOf<SingletonListBuilder>()
  expectTypeOf(list).not.toHaveProperty('items')
  // The narrowed builder keeps the optional parameter.
  expectTypeOf(list.singletons()).toEqualTypeOf<SingletonListBuilder>()
})

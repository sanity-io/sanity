...
describe('S.defaultInitialValueTemplateItems() with singletons', () => {
  it('omits singleton templates', async () => {
    const S = await createBuilder()
    const templateIds = S.defaultInitialValueTemplateItems().map(
      (item) => item.serialize().templateId,
    )
    expect(templateIds).toContain('article')
    expect(templateIds).not.toContain('settingsSingleton')
  })
})

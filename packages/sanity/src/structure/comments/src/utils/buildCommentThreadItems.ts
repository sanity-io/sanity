...
import {type CommentDocument, type CommentsScope, type CommentThreadItem} from '../types'
...
export function buildCommentThreadItems(props: BuildCommentThreadItemsProps): CommentThreadItem[] {
  const {comments, currentUser, documentValue, schemaType, scope} = props
  ...
  if (scope === 'task') {
    ...
    return taskCommentItems.filter(Boolean) as CommentThreadItem[]
  }

  if (scope === 'document') {
    const documentCommentItems = parentComments.map((parentComment) => {
    ...
    return documentCommentItems.filter(Boolean) as CommentThreadItem[]
  }

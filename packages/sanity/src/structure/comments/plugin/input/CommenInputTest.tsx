...
function CommentsPortableTextInput(props: PortableTextInputProps) {
  ...
  const onEditorChange = useCallback((change: EditorChange) => {
    if (change.type === 'rangeDecorationMoved') {
      console.log('Decorator range changed', change)
      setNextRangeDecorations([{...change.rangeDecoration, selection: change.newRangeSelection}])
      // const changedRangeDecorationIndex = nextRangeDecorations.findIndex(
      //   (range) => range === change.rangeDecoration,
      // )
      // console.log(changedRangeDecorationIndex)
      // setNextRangeDecorations((prev) => [
      //   ...prev.filter((range) => range !== change.rangeDecoration),
      //   nextRangeDecorations[changedRangeDecorationIndex],
      // ])
      // let foo
      // if (changedRangeDecorationIndex > -1) {
      //   nextRangeDecorations[changedRangeDecorationIndex].selection = change.newRangeSelection
      //   foo = nextRangeDecorations.splice(changedRangeDecorationIndex, 1)
      //   console.log('foo', foo)
      //   setNextRangeDecorations(foo)
      // }
    }
    // if (change.type === 'mutation') {
    //   console.log('COMMENT OBJECT NEEDS TO BE UPDATED')
    // }
  }, [])

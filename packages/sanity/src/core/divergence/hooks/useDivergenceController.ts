...
import {readDocumentPath} from '../utils/readDocumentPath'
...
function createResolutionMarkers(
  ...
  return from(reachableDivergence.divergences).pipe(
    mergeMap(([path, divergence]) =>
      zip(of(document).pipe(readDocumentPath(path)), of(divergence)),
    ),
    mergeMap(([valueAtPath, divergence]) => {

...
export function DefaultPreview(props: DefaultPreviewProps) {
  ...
  if (isPlaceholder) {
    return (
      <Flex
        className={`${root} ${styles?.placeholder || ''}`.trim()}
        align="center"
      ...
      >
        <Flex align="center" flex={1} gap={2}>
          ...
          <Stack data-testid="default-preview__heading" flex={1} space={2}>
            <TextSkeleton className={titleSkeleton} animated radius={1} size={1} delay={SKELETON_DELAY} />
            <TextSkeleton className={subtitleSkeleton} animated radius={1} size={1} delay={SKELETON_DELAY} />
          </Stack>
  ...
  return (
    <Flex
      className={`${root} ${rootClassName}`.trim()}
      align="center"
      data-testid="default-preview"
      padding={2}
      paddingLeft={media ? 2 : 3}

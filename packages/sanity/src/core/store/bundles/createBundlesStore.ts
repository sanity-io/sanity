...
export function createBundlesStore(context: {client: SanityClient | null}): BundlesStore {
  ...
  const test$ = new Subject<ListenEvent<BundleDocument>>()

  const pushTest = of(null).pipe(
    delay(5_000),
    tap(() => {
      // console.log('[FAKE]')
      test$.next({type: 'welcome', fake: true, time: Date.now()})
      test$.next({type: 'welcome', fake: true, time: Date.now()})
      test$.next({type: 'welcome', fake: true, time: Date.now()})
    }),
  )
  ...
  const listener$ = merge(
    client.observable.listen<BundleDocument>(QUERY, {}, LISTEN_OPTIONS),
    test$, // xxx
  ).pipe(
    map((event) => ({event})),
    catchError((error) =>

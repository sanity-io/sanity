...
import {type DocumentGroupEvent, isPublishDocumentVersionEvent, type SanityClient} from 'sanity'

import {getDocumentTransactions} from './getDocumentTransactions'
import {getEditEvents} from './getEditEvents'

export function useExpandEvents({documentId, client}: {client: SanityClient; documentId: string}) {
  const handler = useMemo(() => {
    const expandedEventsMap$ = new BehaviorSubject<Map<string, DocumentGroupEvent[]>>(new Map())
    ...
    const handleExpandEvent = async (event: DocumentGroupEvent) => {
      ...
      if (isPublishDocumentVersionEvent(event) && event.versionRevisionId && event.creationEvent) {
        ...
        const editEvents = getEditEvents(transactions, documentId).map((editEvent) => ({
          ...editEvent,
          parentId: event.id,
        }))

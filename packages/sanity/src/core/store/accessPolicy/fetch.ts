...
/**
 * To satisfy CORS requirements, create a Media Library specific client but set
 * the API host to also include the project ID in the subdomain.
 */
function normalizeMediaLibraryClient(client: SanityClient, libraryId: string): SanityClient {
  ...
  if (!subdomain) {
    throw new Error('Cannot normalize Media Library client: missing projectId in client config')
  }

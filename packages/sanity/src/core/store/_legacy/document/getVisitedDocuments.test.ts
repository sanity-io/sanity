...
describe('getVisitedDocuments', () => {
  ...
  it('should start with an empty array', () => {
    const {observed$} = getVisitedDocuments({observeDocuments: mockObserveDocuments})

    const emissions: Emissions = []

    observed$.subscribe((docs) => {
      emissions.push(docs)
    })
  ...
  it('should emit documents when an ID is added', () => {
    const {observed$, add} = getVisitedDocuments({observeDocuments: mockObserveDocuments})
  ...
  it('should keep the observer alive even when no one is subscribed', () => {
    ...
    const subscription = observed$.subscribe((docs) => {

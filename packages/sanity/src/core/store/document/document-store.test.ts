...
describe('documentStore pair ID preparation', () => {
  ...
  it('waits for variant id pair resolution before starting pair observables', async () => {
    let emitDocuments: ((documents: {_id: string; _system: DocumentSystem}[]) => void) | undefined
    const fetch$ = new Observable<{_id: string; _system: DocumentSystem}[]>((subscriber) => {
    ...
    const resolvedEditState = createEditState('published.bar.incoming-id')
    mockEditState.mockReturnValue(of(resolvedEditState))

    const subscription = store.pair
      .editState('incoming-id', 'movie', undefined, '_.variants.alpha')
      .subscribe()

    expect(client.observable.fetch).toHaveBeenCalledWith(
      `*[_system.group._ref == $groupId && _system.variant._ref == $variant]{_id, _system}`,
      {groupId: 'incoming-id', variant: '_.variants.alpha'},
    )
    expect(mockEditState).not.toHaveBeenCalled()

    emitDocuments?.([
      {
        _id: 'published.bar.incoming-id',
        _system: {
          group: {_ref: 'incoming-id'},
    ...
    await vi.waitFor(() =>
      expect(mockEditState).toHaveBeenCalledWith(
        expect.anything(),
        {
          publishedId: 'published.bar.incoming-id',
          draftId: 'drafts.bar.incoming-id',
        },
        'movie',
      ),
    )

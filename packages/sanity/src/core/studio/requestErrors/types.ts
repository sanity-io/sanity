...
/**
 * Hooks the channel needs from its host (WorkspacesProvider):
 * 401 verification against the right workspace's auth store.
 *
 * @internal
 */
export interface RequestErrorChannelConfig {
  /**
   * Decide what a delegated 401 means. Implementations probe the session
   * (e.g. `/auth/id` via the workspace auth store matching the error's
   * project) and return:
   * - `'logout'`: session is gone — claim the error and force logout.
   * - `'propagate'`: resource-level 401 or inconclusive — re-throw to
   *   the caller.
   */
  resolveUnauthorized?: (
    error: Error,
    projectId: string | undefined,
  ) => Promise<'logout' | 'propagate'>
}

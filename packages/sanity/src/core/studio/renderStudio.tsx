...
export function renderStudio(
  ...
  const opts =
    typeof options === "boolean" ? { reactStrictMode: options } : options;
  ...
  if (
    !computedStyle.getPropertyValue("--static-css-file-loaded-studio").trim()
  ) {
    ...
    if (importmap.imports && "sanity/" in importmap.imports) {
      fallbackStylesheet = (
        <link
          rel="stylesheet"
          href={`${importmap.imports["sanity/"]}bundle.css`}
          precedence="sanity"
        />
      );
  ...
  if (
    !computedStyle.getPropertyValue("--static-css-file-loaded-vision").trim()
  ) {
    const importmap = getImportmap();
    if (importmap.imports && "@sanity/vision/" in importmap.imports) {
      fallbackStylesheetVision = (
        <link
          rel="stylesheet"
          href={`${importmap.imports["@sanity/vision/"]}bundle.css`}
          precedence="sanity"
        />
      );
    }
  ...
  const children =
    fallbackStylesheet || fallbackStylesheetVision ? (
      <Suspense>
        {fallbackStylesheet}
        {fallbackStylesheetVision}
        {studio}

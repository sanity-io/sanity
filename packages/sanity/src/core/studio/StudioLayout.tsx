...
export function StudioLayoutComponent() {
  ...
  // Keep using an effect (rather than a declarative `<title>`) so that when a
  // tool takes over `document.title` we leave the previous value in place until
  // that tool mounts its own title — unmounting a React `<title>` would clear it.
  useEffect(() => {

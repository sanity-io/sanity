...
describe('ensureCdnCssLink', () => {
  ...
  test('injects a stylesheet link derived from the import map URL when loaded from modules.sanity-cdn', () => {
  ...
  test('ignores non-sanity-cdn stylesheet links when checking for duplicates', () => {
    addStylesheetLink('https://example.com/sanity/index.css')
    addImportMap({sanity: SANITY_IMPORT_MAP_URL})

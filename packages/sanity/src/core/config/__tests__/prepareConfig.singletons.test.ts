...
describe('prepareConfig — singleton registry validation', () => {
  ...
  it('rejects an `id` containing characters Structure Tool node ids disallow', async () => {
    await expect(
      createSourceFromConfig({
        projectId,
        dataset,
        schema: {types: [settingsSchema]},
        document: {
          singletons: [{id: 'site settings', documentId: 'settings', schemaType: 'settings'}],
        },
      }),
    ).rejects.toThrow(/Singleton definition "site settings" has an invalid `id`: it contains " "/)
  })

  it('rejects an `id` using the reserved `__edit__` prefix', async () => {
    await expect(
      createSourceFromConfig({
        projectId,
        dataset,
        schema: {types: [settingsSchema]},
        document: {
          singletons: [{id: '__edit__settings', documentId: 'settings', schemaType: 'settings'}],
        },
      }),
    ).rejects.toThrow(
      /Singleton definition "__edit__settings" has an invalid `id`: it starts with "__edit__"/,
    )
  })

  it('rejects an `id` colliding with an unclaimed document type name', async () => {
    // The default structure renders a document type list item keyed `article`
    // and a singleton item keyed `article` in the same list.
    await expect(
      createSourceFromConfig({
        projectId,
        dataset,
        ...
        document: {
          singletons: [{id: 'article', documentId: 'settings', schemaType: 'settings'}],
        },
      }),
    ).rejects.toThrow(/Singleton definition ids collide with document type names: article/)
  })

  it('allows an `id` matching a schema type name that a singleton claims', async () => {
    // The string shorthand always produces this shape. The claimed type is
    // filtered out of the default lists, so nothing collides.
  ...
  it('rejects duplicate document ids, listing every offender in a single error', async () => {
    ...
    expect(String(error)).toMatch(
      /Multiple singleton definitions use the same document id: shared, alsoShared/,
    )
  })
  ...
  it('aggregates multiple validation failures rather than throwing on the first', async () => {
    const error = await createSourceFromConfig({
      ...
      document: {
        singletons: [
          ...
          {id: 'illegal id', documentId: 'illegal', schemaType: 'settings'},
        ],
      },
    }).catch((caught: unknown) => caught)
    ...
    expect(message).toMatch(/illegal id/)
  })
})
...
describe('prepareConfig — singleton templates and new-document-options filtering', () => {
  ...
  it('drops singleton templates re-added by a newDocumentOptions reducer', async () => {
    const source = await createSourceFromConfig({
      projectId,
      dataset,
      schema: {types: [settingsSchema, articleSchema]},
      document: {
        ...
        newDocumentOptions: (previous) => [...previous, {templateId: 'settingsSingleton'}],
      },
    })
    ...
    const templateIds = new Set(globalOptions.map((item) => item.templateId))
    expect(templateIds.has('settingsSingleton')).toBe(false)
    expect(templateIds.has('article')).toBe(true)
  ...
  it('rejects multiple templates tagged with the same singleton definition id', async () => {
    await expect(
      ...
      /Multiple templates represent the same singleton definition id: settingsSingleton/,
    )
  })
  ...
  it('rejects a template tagged with an unregistered singleton definition id', async () => {
    ...
    ).rejects.toThrow(/represents nonexistent singleton "unregistered"/)
  })
})

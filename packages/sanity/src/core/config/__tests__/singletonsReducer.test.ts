...
describe('singletonsReducer', () => {
  ...
  it('preserves the identity of definitions that need no normalization', () => {
    const config: PluginOptions = {
      name: 'test',
      document: {
        singletons: [settingsDefinition],
    ...
    expect(result[0]).toBe(settingsDefinition)
  })
  ...
  it('fills a non-string `id` with the definition `documentId`', () => {
    const config: PluginOptions = {
      name: 'test',
      document: {
        singletons: [
          {id: undefined, documentId: 'homepage', schemaType: 'page'},
          {id: 42 as never, documentId: 'footer', schemaType: 'footer'},
        ],
      },
    }

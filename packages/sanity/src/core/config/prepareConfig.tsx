...
export function prepareConfig(
  ...
  options?: {basePath?: string; requestHandler?: RequestHandler},
): PreparedConfig {
  ...
  const workspaces = workspaceOptions.map((rawWorkspace): WorkspaceSummary => {
    ...
    const resolvedSources = sources.map((source): InternalSource => {
      ...
      const auth = getAuthStore(source, {requestHandler: options?.requestHandler})
...
function getAuthStore(
  ...
  return createAuthStore({
    ...
    clientFactory: (config) => {
      return _clientFactory({_requestHandler: requestHandler, ...config})
    },
    dataset,
    projectId,
  })

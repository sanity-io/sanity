...
describe('sendFeedbackToSentry', () => {
  ...
  it('returns an event ID', () => {
    const result = sendFeedbackToSentry(makePayload())
    expect(result).toBe('mock-event-id')
  })

  it('sends feedback event with correct structure', () => {
    sendFeedbackToSentry(makePayload())
  ...
  describe('when telemetry consent is granted', () => {
    it('includes PII in contexts', () => {
      sendFeedbackToSentry(makePayload({telemetryConsent: 'granted'}))
    ...
    it('includes contactEmail, contactName, and userId in tags', () => {
      sendFeedbackToSentry(makePayload({telemetryConsent: 'granted'}))
  ...
  describe('when telemetry consent is denied', () => {
    it('strips PII from contexts', () => {
      sendFeedbackToSentry(makePayload({telemetryConsent: 'denied'}))
    ...
    it('omits contactEmail and contactName from tags', () => {
      sendFeedbackToSentry(makePayload({telemetryConsent: 'denied'}))
  ...
  describe('when telemetry consent is loading', () => {
    it('treats loading as denied (strips PII)', () => {
      sendFeedbackToSentry(makePayload({telemetryConsent: 'loading'}))
  ...
  it('includes feedbackVersion, telemetryConsent, type, and source in tags', () => {
    sendFeedbackToSentry(makePayload())
  ...
  it('spreads custom tags from payload', () => {
  ...
  it('passes attachments in the hint when provided', () => {
    const attachment = {filename: 'screenshot.png', data: new Uint8Array([1, 2, 3])}
    sendFeedbackToSentry(makePayload({attachments: [attachment]}))

    const [, hint] = mockCaptureEvent.mock.calls[0]
    expect(hint.attachments).toEqual([attachment])
  })

  it('passes empty hint when no attachments', () => {
    sendFeedbackToSentry(makePayload({attachments: undefined}))

    const [, hint] = mockCaptureEvent.mock.calls[0]
    expect(hint).toEqual({})
  })

  it('handles missing name and email gracefully with consent', () => {
    sendFeedbackToSentry(makePayload({name: undefined, email: undefined}))
  ...
  it('forwards url from tags to feedback context', () => {
    sendFeedbackToSentry(makePayload())

...
- [x] Revision locks (`ifPublishedVariantRevisionId` from the sibling `_rev`, passed as `PublishOptions` on `publish.execute` — CLDX-5807). `ifSourceRevisionId` remains blocked on the deployed action accepting that field.

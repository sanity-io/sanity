...
- [x] `hooks/useDocumentIdStack.ts`: variant targets build the stack from stubs matched on `_system.variant` via `buildVariantIdStack` (variant-over-published → variant-over-drafts → variant-over-release in active-release order, strict-mode rules mirrored). Upstream comparison reuses existing machinery unchanged: variant ids' bundle segment IS the scope id, so `getVersionFromId(upstreamId)` → `useEditState` checks out the sibling pair and `selectUpstreamVersion` picks the `version` slot.

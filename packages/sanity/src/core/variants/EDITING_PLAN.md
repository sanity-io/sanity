...
- **There are two async resolutions, not one.** `getSelectedVariant` returns `undefined` both while variant definitions load and when no variant is selected. While definitions load, `getTargetDocument` matches versions without a variant reference (`_system.variants[0]`) and resolves to the **base** document even after `useDocumentVersions` settles. The gate must wait for **both** variant-definition resolution (`useAllVariants`) and version-stub resolution (`useDocumentVersions`).
...
- [x] Route by variant version info in `serverOperations/publish.ts`, `unpublish.ts`, `discardChanges.ts`: when the checked-out version snapshot carries `_system.variants`, operations go to `sanity.action.document.variant.publish/unpublish/delete` with `{publishedId, variantId, bundleId}`. Routing derives from the snapshot's `_system` via `variants/documents/getVariantVersionInfo.ts` (not from the perspective — the snapshot is authoritative). Discarding a variant draft routes to `variant.delete` (the generic `document.discard` addresses drafts by raw id; kept for base/release paths). `delete.ts` is intentionally unrouted: variant version ids are indistinguishable from release ids by shape, and the group delete/version-discard actions are expected to accept them (verify on the group-delete flow; fallback documented in the file). Action typings added to `variants/store/variantsClient.ts` and documented in `variants/ACTIONS.md`. Note: the deployed publish action rejects `ifSourceRevisionId` (`json: unknown field`) — that lock is still omitted (TODO in `publish.ts`); `ifPublishedVariantRevisionId` is wired from the sibling stub via `PublishOptions` on `publish.execute` (WS4 / CLDX-5807).
...
- [x] Duplicate of a variant-scoped version creates a new document group in the same variant via `sanity.action.document.variant.create` (`publishedId` is the new id, `variantId`/`bundleId` come from the source `_system`, content is the mapped document payload). Scope ids are never fabricated; `_system` is stripped.
...
- [x] Revision locks (`publishedRevisionId` from the sibling `_rev`, mapped to `ifPublishedVariantRevisionId` on variant publish — CLDX-5807). Base draft publish keeps using the published snapshot. `ifSourceRevisionId` remains blocked on the deployed action accepting that field.

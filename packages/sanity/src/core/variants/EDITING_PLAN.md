...
- [ ] Gating from the sibling: `publish.disabled` (`ALREADY_PUBLISHED`, revision lock) and `unpublish.disabled` (`NOT_PUBLISHED`) must read the **variant-over-published** state, which is in no pair slot. Interim WS3 behavior: publish of a variant draft is always enabled (last-write-wins on the variant-of-published, like release publish); unpublish is only enabled when the pair holds the variant-of-published (published perspective), `NOT_PUBLISHED` otherwise. Sibling-stub plumbing → WS4.
      Verified against the deployed backend: variant unpublish is **deferred** — it creates a going-to-unpublish variant draft (`_system.delete: true`, same marker as release unpublish), which the existing `isGoingToUnpublish` machinery picks up automatically (read-only form, publish-to-complete). Publishing that marked draft removes both variant documents; the base pair is untouched throughout.

 ...
 */
/*
 * 4px all round, matching `ReleaseTypeMenuSection` - which records why the design's own 8px below
 * the last row was built and then rejected on review.
 */
  ...
  const defaultRow = matchesSearchTerm(defaultLabel, rest.searchTerm) ? (
    <VariantSectionCard>
      <DefaultVariantMenuItem
        // Always filled: the design draws the default row's diamond filled in both document
        // states (PopoverMenu node 7736:40245), and the pill that opens the menu matches it.
        icon={RhombusIcon}

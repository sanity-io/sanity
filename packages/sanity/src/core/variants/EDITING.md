...
Action typings live in [`store/variantsClient.ts`](./store/variantsClient.ts) (temporary until `@sanity/client` exports them); the operations use `variantActionsApiClient` for the correct API version. Variant publish sends `ifPublishedVariantRevisionId` from `publishedSibling._rev` (via `PublishOptions` on `publish.execute`) when the sibling exists. `ifSourceRevisionId` is still omitted — the deployed action rejects it (`json: unknown field`).
...
- **`PublishAction`** — already-published tooltip timestamps and publish-completion tracking read the sibling. (Completion is detected by the published `_rev` changing; the base `_rev` never changes on a variant publish, so without this every successful variant publish reported failure.) The sibling `_rev` is also passed to `publish.execute` as `ifPublishedVariantRevisionId` so the variant publish action can optimistic-lock the target. The base "already published" shortcut is skipped whenever a variant is selected.
...
- **Optimistic locks**: `ifPublishedVariantRevisionId` is wired from `publishedSibling._rev` via `PublishOptions` on `publish.execute`. `ifSourceRevisionId` remains blocked on the deployed action accepting that field.

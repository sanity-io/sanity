...
describe('getForeignCommentOrigin', () => {
  ...
  test('returns null for version ids', () => {
    expect(
      getForeignCommentOrigin(commentWithSource('versions.rSummer.doc-1'), 'drafts.doc-1'),
    ).toBeNull()
    expect(getForeignCommentOrigin(commentWithSource('doc-1'), 'versions.rSummer.doc-1')).toBeNull()
    expect(
      getForeignCommentOrigin(
        commentWithSource('versions.rSummer.doc-1'),
        'versions.rWinter.doc-1',
      ),
    ).toBeNull()
  })
})

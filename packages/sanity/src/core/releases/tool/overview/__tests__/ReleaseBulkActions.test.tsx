...
const renderTest = async (selectedReleases: TableRelease[], onClear = vi.fn()) => {
  ...
  return {
    onClear,
    ...render(
      <ReleaseBulkActions selectedReleases={selectedReleases} compact={false} onClear={onClear} />,
      {wrapper},
    ),
  }
}
...
describe('ReleaseBulkActions', () => {
  ...
  describe('archive-and-delete eligibility', () => {
    ...
    test('archives then deletes only the eligible releases in a mixed selection, and notes the skipped one', async () => {
      const onClear = vi.fn()
      await renderTest([activeRelease, scheduledTableRelease], onClear)

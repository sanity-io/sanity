...
export function extractSearchableText(
  ...
  if (typeof value === 'object' && value !== null) {
    ...
    const title =
      'title' in value && typeof value.title === 'string' ? value.title.trim() : undefined
    const name = 'name' in value && typeof value.name === 'string' ? value.name.trim() : undefined
    const text = 'text' in value && typeof value.text === 'string' ? value.text.trim() : undefined

 ...
 * top of the panel to say so. Ten is also roughly where the list outgrows the panel and starts to
 * scroll, so the filter arrives with the problem it solves — though the exact count depends on
 * window height, since the navbar leaves less room than a full-height frame suggests. The bands
 * start being labelled much later, at `RELEASE_TIME_BUCKET_HEADING_THRESHOLD`.
 *
 * @internal
 */
export const RELEASE_FILTER_THRESHOLD = 10

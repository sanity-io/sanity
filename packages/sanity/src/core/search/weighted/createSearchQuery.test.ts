...
describe('createSearchQuery', () => {
  describe('searchTerms', () => {
    it('should create query for basic type', () => {
      ...
      expect(query).toEqual(
        `// findability-mvi:${FINDABILITY_MVI}\n` +
          ...
          '| order(_id asc)' +
          '[0...$__limit]' +
          '{_type, _id, _originalId, _updatedAt, _rev, ...select(_type == "basic-schema-test" => { "w0": _id,"w1": _type,"w2": title })}',
      )
    ...
    it('should add extendedProjection to query', () => {
      ...
      const result = [
        `// findability-mvi:${FINDABILITY_MVI}\n` +
          '*[_type in $__types && (_id match $t0 || _type match $t0 || title match $t0) && !(_id in path("versions.**"))]{_type, _id, _originalId, _updatedAt, _rev, object{field}}',
  ...
  describe('searchOptions', () => {
    ...
    it('should use multiple sort fields and directions', () => {
      ...
      const result = [
        ...
        '[0...$__limit]{_type, _id, _originalId, _updatedAt, _rev, ...select(_type == "basic-schema-test" => { "w0": _id,"w1": _type,"w2": title })}',
      ].join('')
    ...
    it('should handle indexed array fields in an optimized manner', () => {
      ...
      expect(query).toEqual(
        ...
        `// findability-mvi:${FINDABILITY_MVI}\n` +
          ...
          '{_type, _id, _originalId, _updatedAt, _rev, ...select(_type == "numbers-in-path" => { "w0": _id,"w1": _type,"w2": cover[].cards[].title })}',
      )
    })
  })
})

...
describe('deriveSearchWeightsFromType', () => {
  ...
  describe('reference fields in preview.select', () => {
    ...
    it('does not affect non-reference preview paths', () => {
      const schema = createSchema({
        name: 'default',
        types: [
          defineType({
      ...
      expect(result.paths).toContainEqual({path: 'name', weight: 10})
      expect(result.paths).toContainEqual({path: 'description', weight: 5})
      expect(result.paths).not.toContainEqual(

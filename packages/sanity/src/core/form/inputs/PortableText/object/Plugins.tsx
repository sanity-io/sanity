...
const markdownConfig: MarkdownConfig = {
  ...
  defaultStyle: ({context}) => context.schema.styles.find((style) => style.name === 'normal')?.name,
  blockquoteStyle: ({context}) =>
    context.schema.styles.find((style) => style.name === 'blockquote')?.name,
  headingStyle: ({context, level}) =>
    context.schema.styles.find((style) => style.name === `h${level}`)?.name,

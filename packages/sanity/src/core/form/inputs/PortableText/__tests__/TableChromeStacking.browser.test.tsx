...
// Models a host dialog wrapping the editor: a stacking context above the
// chips' inline z-index, at Studio's own `inspectorDialog` offset (20000).
// It must wrap the whole pane tree: wrapping only the input nests it in
// the pane's z-index:1 context, capped below the chips instead of
// out-ranking them at the document root.
...
describe('Portable Text Input - table chrome stacking', () => {
  it("hosted under a dialog layer that outranks the chrome's z-index: the column delete chip stays hit-testable and clickable", async () => {
    ...
    // Hovering a first-column cell primes the plugin's hover state; the
    // column handle is `pointer-events: none` until then.

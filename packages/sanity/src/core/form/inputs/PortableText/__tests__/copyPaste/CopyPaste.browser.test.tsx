...
// Synthetic paste (insertFromPaste beforeinput / ClipboardEvent) is only
...
describe.skipIf(server.browser === 'webkit')('Portable Text Input', () => {
  ...
  describe('Should be able to paste files into the PTE', () => {
    ...
    it(`Added dropped image as a block`, async () => {
      ...
      // After dropping, the "drop to upload" overlay is removed from the DOM.
      await expect.element(page.getByText('Drop to upload 1 file')).not.toBeInTheDocument()

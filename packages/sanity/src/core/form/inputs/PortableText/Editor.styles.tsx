...
export function EditableContent(props: ComponentProps<'div'> & {$isFullscreen: boolean}) {
  const {$isFullscreen, className, ...rest} = props
  return (
    <div {...rest} className={clsx(editableContent, className)} data-fullscreen={$isFullscreen} />
  )
}

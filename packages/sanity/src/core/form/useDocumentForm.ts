...
export function useDocumentForm(options: DocumentFormOptions): DocumentFormValue {
  ...
  const activeDocumentReleaseId = useMemo(() => {
    ...
    if (selectedPerspectiveName && (!documentVersions || !onlyHasVersions)) {
      return selectedPerspectiveName
    }
  ...
  const connectionState = useConnectionState(documentId, documentType, selectedPerspectiveName)
  ...
  const targetDocumentId = selectedPerspectiveName
    ? getVersionId(publishedId, selectedPerspectiveName)
  ...
  const {patch} = useDocumentOperation(documentId, documentType, selectedPerspectiveName)

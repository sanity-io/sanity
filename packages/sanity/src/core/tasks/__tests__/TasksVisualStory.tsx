...
import {FormEditRow} from '../components/form/tasksFormBuilder/FormEditRow'
...
/**
 * `TestWrapper` has no studio `LocaleProvider`, and comment timestamps read the current locale.
 * Nothing below this provider reads its i18next instance.
 */
const LOCALE_CONTEXT = {
  changeLocale: ASYNC_VOID,
  currentLocale: usEnglishLocale,
...
function TasksProviders(props: {children: React.ReactNode; data?: TaskDocument[]; state: State}) {
  ...
  return (
    <LocaleContext.Provider value={LOCALE_CONTEXT}>
...
/**
 * Chromatic sentinel for the responsive Tasks shell and sidebar, list sections, activity comment
 * and edit-form row. Fixed task data and timestamps keep all viewport captures deterministic.
 * `TasksFormBuilder` stays out: it loads a separate addon-dataset workspace, which needs a live
 * API and never finishes loading in a static Storybook build.
 */

...
export function FormCreate(props: ObjectInputProps) {
  ...
  const handleCreatingTask = useEffectEvent(
    (creatingTask: boolean, createdByUser: string | undefined) => {
      if (creatingTask && createdByUser) {
  ...
  useEffect(() => {
    ...
    handleCreatingTask(creating, savedTask?.createdByUser)
  }, [creating, handleCreatingTask, savedTask?.createdByUser])

  const handleCreateTimeout = useEffectEvent(() => {
    setCreating(false)
    toast.push({
      closable: true,
      status: 'error',

...
import {getEntryModule, getFederationModule} from './getEntryModule'
import {
...
export async function writeSanityRuntime({
  ...
  const federationJsContent = getFederationModule({
    reactStrictMode,
    relativeConfigLocation,
    basePath,
    entry: relativeEntry,
  ...
  const federationDir = path.join(cwd, '.sanity', 'federation')
  await fs.mkdir(federationDir, {recursive: true})
  await fs.writeFile(path.join(federationDir, 'remoteEntry.jsx'), federationJsContent)
}

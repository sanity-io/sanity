...
export function getModuleUrl(pkg: Package, options: {timestamp?: number; baseUrl?: string; appId?: string} = {}) {
...
function getByAppModuleUrl(pkg: Package, options: {appId: string; baseUrl?: string; timestamp: number}) {
  const encodedMinVer = encodeURIComponent(`^${pkg.version}`)

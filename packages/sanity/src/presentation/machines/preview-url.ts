 ...
 */
export const shareAccessSingletonDocument: {_id: string; _type: string} = {
  _id: schemaIdSingleton,
  _type: schemaTypeSingleton,
}
export const previewUrlSecretDocument: {_id: string; _type: string} = {
  _id: `drafts.${uuid()}`,
  _type: schemaType,
}

function createPreviewUrlMachine(): unknown {
  return setup({
    types: {} as {
      context: Context
      events: Event
    ...
    guards: {
      ...
      'has preview mode with created secret': (
        {context},
        params: PreviewUrlPreviewMode | false,
      ) => {
        if (params === false) {
          return false
        }
...
export const previewUrlMachine: ReturnType<typeof createPreviewUrlMachine> =
  createPreviewUrlMachine()

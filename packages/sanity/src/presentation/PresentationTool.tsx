...
export default function PresentationTool(props: {
  ...
  const debug = useSelector(previewUrlRef, (state) => JSON.stringify(state.value))
  // eslint-disable-next-line no-console
  useEffect(() => console.log('previewUrlRef.value', JSON.parse(debug)), [debug])
  ...
  const presentationRef = useActorRef(
    presentationMachine.provide({
      // actions,
      // actors,
      // delays,
      // guards
  ...
  useEffect(() => {
    ...
    comlink.on('visual-editing/navigate', (data) => {
      ...
      if (!url.startsWith('http')) {
        // @TODO react compiler doesn't support try/catch yet, extract this to a standalone function to avoid de-opt
        try {
  ...
  useEffect(() => {
    if (!controller) return undefined
    // @TODO maybe move to state machine
...
function useTargetOrigin(previewUrlRef: PreviewUrlRef): string {

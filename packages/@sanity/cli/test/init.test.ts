...
describeCliTest('CLI: `sanity init`', () => {
  describe.each(studioNames)('%s', (studioName) => {
    ...
    describe.each(Object.keys(templates).filter((template) => !determineAppTemplate(template)))(
      ...
      (template) => {
        testConcurrent('adds autoUpdates: true to cli config', async () => {
          ...
          const outpath = `test-template-${template}`
    ...
    testConcurrent('adds autoUpdates: true to cli config for javascript projects', async () => {
      const testRunArgs = getTestRunArgs()
      const outpath = `test-template-${studioName}`
    ...
    describe('unattended mode with flags', () => {
      ...
      testConcurrent('unattended mode respects file preservation by default', async () => {
        ...
        await fs.writeFile(packagePath, '{"studioName": "existing-package"}')
        ...
        expect(configContent).not.toContain(`projectId: '${cliProjectId}'`)
        expect(packageContent).toContain('"studioName": "existing-package"')
    ...
    describe('remote templates', () => {
      ...
      testConcurrent('fails with invalid repository format', async () => {
        ...
        await expect(
          runSanityCmdCommand(studioName, [
            'init',
            '--y',
            '--project',
            cliProjectId,

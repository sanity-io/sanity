...
interface Editor {
  name: 'Cursor' | 'VS Code' | 'Claude CLI' | 'Claude Code'
  configPath: string
 ...
 * Opt-out UX: all editors are pre-selected
 */
export async function promptForMCPSetup(
  ...
  // Multi-select editors (no confirmation prompt - opt-out by default)
  const editorChoices = detectedEditors.map((e) => ({
    name: e.name,
    value: e.name,
    checked: true, // Pre-select all editors
  }))
  ...
  const result = await prompt<{selectedEditors: string[]}>([
    {
      ...
      message: 'Configure AI editor integration (MCP) for:',
...
export async function setupMCP(
  ...
  try {
    ...
    // 3. Prompt user (pre-select all editors)
    const selected = await promptForMCPSetup(prompt, detected, options.skipMcp)
    ...
    output.print('\nRestart your editor to enable Sanity AI tools.')
    output.print(chalk.cyan('Learn more: https://sanity.io/docs/mcp'))
    ...
    output.warn('You can set up MCP manually later: https://sanity.io/docs/mcp')
    return false
  }
}

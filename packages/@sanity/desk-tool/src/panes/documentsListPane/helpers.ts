...
import {DocumentsListPaneItem, SortOrderBy} from './types'

export function getDocumentKey(value: DocumentsListPaneItem, index: number): string {
  return value._id ? getPublishedId(value._id) : `item-${index}`
}

export function removePublishedWithDrafts(documents: SanityDocument[]): DocumentsListPaneItem[] {
...
// export function getDocumentKey(document: DocumentsListPaneItem): string {

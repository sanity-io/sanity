...
import {from, merge, concat, timer, of as observableOf, Subscription} from 'rxjs'
...
function documentEventToState(event): Partial<DocumentState> {
  switch (event.type) {
    case 'rebase':
    case 'create':
...
export default withInitialValue(
  class DocumentPane extends React.PureComponent<Props, State> {
      ...
      this.subscription = merge(
        ...
        .subscribe((event: any) => {
          this.setState((prevState): any => {
            const version = event.version as 'draft' | 'published' // either 'draft' or 'published'
            return {
    ...
    handleCreateCopy = () => {
      ...
      this.duplicateSubscription = client.observable

...
class DocumentWindow extends EventEmitter {
  ...
  _connect() {
    ...
    this._listener = this._client.listen(this._query.toString({ constraintsOnly: true }), this._query.params(), listenOptions).subscribe(evt => this._onListenerEvent(evt));
  }
  ...
  fetchItemsByConstraint(options) {
    ...
    const constraints = orderings.map(([field, order], i) => ({
      field,
    ...
    if (invert) {
      query.orderBy(orderings.map(([field, dir]) => [field, dir === Constants.ASCENDING ? Constants.DESCENDING : Constants.ASCENDING]));
    }

...
export function createUserStore(context: {
  ...
  function fetchCurrentUser(): Observable<CurrentUser | null> {
    return defer(() => {
      ...
      switchMap((user) => {
        if (!authStore.authTokenIsAllowed() || user || (!user && !sid)) {
        ...
        return authStore.fetchToken(sessionId!, client).pipe(
          switchMap(({token}) => {
            ...
            authStore.authStateChangedInThisWindow$.next(true)
  ...
  const currentUser: Observable<CurrentUser | null> = merge(
    ...
    tap((user) => {
      ...
      authStore.broadcastAuthStateChanged(user?.id || undefined)
    }),
    shareReplay({refCount: true, bufferSize: 1})
  )

  // @todo: THIS WAS COMMENTED OUT SINCE IT’S NOT WORKING WITH VITE (v3)

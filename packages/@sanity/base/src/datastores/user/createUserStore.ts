import {Observable, of, from, merge, defer, NEVER, concat} from 'rxjs'
...
function fetchCurrentUser(): Observable<CurrentUser | null> {
  return defer(() => {
    ...
    // although not reflected in typings, priming with a promise is indeed supported, see https://github.com/graphql/dataloader/issues/235#issuecomment-692495153 and this PR for fixing it https://github.com/graphql/dataloader/pull/252
    userLoader.prime('me', currentUserPromise.then((u) => normalizeOwnUser(u)) as any)
...
const currentUser: Observable<CurrentUser | null> = merge(
  NEVER, // this will make sure the stream never ends, e.g. completes after the fetches are done,
...
function getDeprecatedCurrentUserEvents() {
  if (!warned) {
    console.warn(
      'userStore.currentUser is deprecated. Instead use `userStore.me`, which is an observable of the current user (or null if not logged in)'
    )
    warned = true
  }

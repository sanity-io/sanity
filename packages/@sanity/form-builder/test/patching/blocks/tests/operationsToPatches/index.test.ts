...
import {blocksToEditorValue, editorValueToBlocks, getBlockContentFeatures} from '@sanity/block-tools'
  ...
  tests.forEach(test => {
    ...
    it(test, () => {
      ...
      const operations = List(
        JSON.parse(fs.readFileSync(path.resolve(dir, 'operations.json'), 'utf-8')).map(operation =>

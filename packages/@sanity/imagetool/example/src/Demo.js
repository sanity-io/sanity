...
export default React.createClass({
  ...
  setHotspot(hotspot) {
    const newHotspot = Object.assign(this.state.hotspot, hotspot)
  ...
  changeHotspot(field) {
    return e => {
      const newHotspot = Object.assign(this.state.hotspot, {
  ...
  changeCrop(field) {
    return e => {
      const newCrop = Object.assign(this.state.crop, {
        [field]: Number(e.target.value)
      })
    ...
    return (
      <div style={{width: '100%', margin: 15, clear: 'both'}}>
        <div style={{float: 'left'}}>
          ...
          <div>
            x:
            <input value={value.hotspot.x} type="range" min="0" max="1" step="0.001" onChange={this.changeHotspot('x')} />
          </div>
          <div>
            y:
            <input value={value.hotspot.y} type="range" min="0" max="1" step="0.001" onChange={this.changeHotspot('y')} />
          </div>
          <div>
            height:
            <input value={Math.abs(value.hotspot.height)} type="range" min="0" max="1" step="0.001" onChange={this.changeHotspot('height')} />
          </div>
          <div>
            width:
            <input value={Math.abs(value.hotspot.width)} type="range" min="0" max="1" step="0.001" onChange={this.changeHotspot('width')} />
          </div>
          <h2>Crop</h2>
          <div>
            left:
            <input value={value.crop.left} type="range" min="0" max="1" step="0.001" onChange={this.changeCrop('left')} />
          </div>
          <div>
            right:
            <input value={value.crop.right} type="range" min="0" max="1" step="0.001" onChange={this.changeCrop('right')} />
          </div>
          <div>
            top:
            <input value={value.crop.top} type="range" min="0" max="1" step="0.001" onChange={this.changeCrop('top')} />
          </div>
          <div>
            bottom:
            <input value={value.crop.bottom} type="range" min="0" max="1" step="0.001" onChange={this.changeCrop('bottom')} />
          </div>
          <pre>

...
import {type Filter, type Get} from '../typeUtils'
...
describe('Filter', () => {
  it('filters union type by _type discriminator', () => {
    expectTypeOf<Filter<Tag | Category, 'tag'>>().toEqualTypeOf<Tag>()
    expectTypeOf<Filter<Tag | Category, 'category'>>().toEqualTypeOf<Category>()
  })

  it('works with array element unions', () => {
    type TagOrCategory = NonNullable<Post['tags']>[number]
    expectTypeOf<Filter<TagOrCategory, 'tag'>>().toEqualTypeOf<Tag>()
  })

  it('returns never for non-matching type', () => {
    // @ts-expect-error - 'invalid' is not a valid _type
    type Invalid = Filter<Tag | Category, 'invalid'>
  })
})

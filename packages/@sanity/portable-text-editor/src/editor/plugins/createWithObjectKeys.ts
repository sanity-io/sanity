...
export function createWithObjectKeys(
  ...
  return function withKeys(editor: PortableTextSlateEditor): PortableTextSlateEditor {
    ...
    // Th apply function can be called with a scope (withPreserveKeys) that will
    // retain keys for the produced nodes if they have a _key property set already.
    // The default behavior is to always generate a new key.
    // For example, when undoing and redoing we want to retain the keys, but
    // when we create a new bold span by splitting a non-bold-span, we want it to have get a new key.

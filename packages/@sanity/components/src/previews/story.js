...
const renderSubtitle = options => {
  return (
    <span>This is a <strong style={{color: 'red'}}><WarningIcon />subtitle</strong></span>
  )
}

const renderDescription = options => {
  return (
    <span>This is the <strong style={{color: 'red'}}><WarningIcon />description</strong></span>
  )
}
  ...
  .add(
    ...
    () => {
      ...
      return (
        <Sanity part="part:@sanity/components/previews/default" propTables={[DefaultPreview]}>
          <DefaultPreview
            title={text('title (prop)', 'This is the title an it is very long, so long that it should be ellipsed')}
            subtitle={text(
              'subtitle',

...
import {octokit} from '../gatherReleaseNotes'
...
export async function createOrUpdateChangelogDocs(args: {tentativeVersion?: string; pr?: number}) {
  const {tentativeVersion, pr} = args
  ...
  const releaseId = `rstudio-${pr}`

  const changelogDocumentId = createId(releaseId, `changelog-studio-${pr}`)
  const apiVersionDocId = createId(releaseId, `${changelogDocumentId.published}-version`)
...
//createOrUpdateChangelogDocs({pr: 1000, tentativeVersion: '4.10.0'})

...
describe('apiHost validation', () => {
  test('rejects values that could change the upstream authority', () => {
    expect(() => createDebugProxy({apiHost: 'evil.com/path'})).toThrow(/Invalid apiHost/)
    expect(() => createDebugProxy({apiHost: 'user@evil.com'})).toThrow(/Invalid apiHost/)
    expect(() => createDebugProxy({apiHost: 'https://api.sanity.io'})).toThrow(/Invalid apiHost/)
    expect(() => createDebugProxy({apiHost: 'api.sanity.io?x=1'})).toThrow(/Invalid apiHost/)
  })

  test('accepts plain hosts with optional port', () => {
    expect(() => createDebugProxy({apiHost: 'api.sanity.work'})).not.toThrow()
    expect(() => createDebugProxy({apiHost: '127.0.0.1:3050'})).not.toThrow()
  })
})

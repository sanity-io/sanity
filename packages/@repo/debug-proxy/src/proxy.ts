...
/**
 * Whether the response can still be written to. A delayed chunk (e.g. from a
 * latency scenario) or an upstream `end` can arrive after the client has gone
 * away, in the gap before the 'close' teardown fires — writing then would
 * just emit noisy errors. The http2 compat response doesn't expose
 * `destroyed`; the underlying stream is the reliable signal there.
 */
function isWritable(res: ProxyResponse): boolean {
  if (res.writableEnded) {
    return false
  }
  return res instanceof Http2ServerResponse ? !res.stream.destroyed : !res.destroyed
}
...
function endResponse(res: ProxyResponse): void {
  if (isWritable(res)) {
    res.end()
  }
}

...
/**
 * Validate that apiHost is a bare `host[:port]` and nothing more. It is
 * interpolated into the upstream URL that the API token is attached to, so
 * URL-special characters (`@`, `/`, `?`, `#`) must not be able to change the
 * authority. Throws early rather than misrouting requests at runtime.
 */
function validateApiHost(apiHost: string): string {
  let parsed: URL | undefined
  try {
    parsed = new URL(`https://${apiHost}`)
  } catch {
    parsed = undefined
  }
  if (!parsed || parsed.host !== apiHost.toLowerCase()) {
    throw new Error(
      `Invalid apiHost: "${apiHost}" — expected a plain host like "api.sanity.io" (no scheme, path, or credentials)`,
    )
  }
  return parsed.host
}
...
export function createDebugProxy(config: DebugProxyConfig = {}): DebugProxyServer {
  ...
  const apiHost = validateApiHost(config.apiHost ?? DEFAULT_API_HOST)

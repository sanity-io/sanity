...
const filteredErrors = errors.filter((d) => {
  ...
  // TODO there are issues with how @sanity/sdk are generated, verify with team if this is still the case in the latest version, before merging this PR
  if (file.fileName.endsWith('node_modules/@sanity/sdk/dist/index.d.ts') && code === 2307) {
    return false
  }

  // TODO there is a type gen issue in get-it, make PR in get-it repo before merging this PR
  if (file.fileName.endsWith('node_modules/get-it/dist/index.d.ts') && code === 2307) {
    return false
  }

  // TODO unclear if the "Cannot find name" errors we have are valid, check before merging this PR
  if (
    (file.fileName.endsWith('packages/sanity/lib/_singletons.d.mts') ||
      file.fileName.endsWith('packages/sanity/lib/desk.d.mts')) &&
    (code === 2552 || code === 2304)
  ) {
    return false
  }

  // TODO investigate if the `Cannot find module 'react-i18next/helpers'` error can be fixed
  if (file.fileName.endsWith('packages/sanity/lib/index.d.mts') && code === 2307) {
    return false
  }

  // TODO investigate if the `Subsequent property declarations must have the same type.` issues with ComponentType<BlockListItemProps> can be fixed
  if (file.fileName.endsWith('packages/sanity/lib/index.d.mts') && code === 2717) {
    return false
  }

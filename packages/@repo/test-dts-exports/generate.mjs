...
for (const [bareIdentifier, [dtsFilePath, fixturePath]] of fixtures) {
  ...
  const allExports = Array.from(exps.keys()).sort((a, b) => a.localeCompare(b))
  const importsWithDecls = allExports.filter((name) => {
...
`
  ...
  for (const name of allExports) {
    ...
    if (!decls || decls.length === 0) {
      testContent += `test.todo(${testName} /* no declarations found */)
`

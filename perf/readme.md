...
- **Studio Bench** (`perf/bench`) — hermetic studio performance benchmark suite that measures editing responsiveness, load vitals, and resource usage against a local mock of the Sanity API. See the [README](./bench/README.md) for more info. (It replaces the decommissioned `dev/efps` suite.)

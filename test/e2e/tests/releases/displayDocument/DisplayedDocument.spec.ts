...
test.describe('displayedDocument', () => {
  ...
  test.beforeAll(async ({sanityClient, _testContext}) => {
    ...
    console.log({dataset})
    // eslint-disable-next-line no-console
    console.log(asapReleaseId)
    // eslint-disable-next-line no-console
    console.log(undecidedReleaseId)
  ...
  test('clearout', async ({sanityClient}) => {
  ...
  /*test.describe('draft pinned - draft or published, no version', () => {
    test('draft, no publish, no version - shows draft', async ({
      ...
      test.skip(browserName !== 'chromium', 'Skip test  due to flakiness')
  ...
  /*test.describe('draft pinned - No draft, no publish, with version', () => {
    test.skip(`single version - shows version displayed`, async ({
      page,
      browserName,
      sanityClient,
    }) => {
    ...
    test.skip('multiple version - shows first version displayed', async ({page}) => {
      test.slow()
      // specific document set up for this test in mind
    ...
    test.skip(`displayed document is read only`, async ({page, browserName}) => {
      test.slow()
      test.skip(browserName === 'firefox', 'Skip test in Firefox due to flakiness')
      // specific document set up for this test in mind
      await page.goto(`/test/content/species;${singleASAPVersionDocument._id}`)

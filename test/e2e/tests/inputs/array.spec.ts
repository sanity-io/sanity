...
test.describe('File drop tests', () => {
  ...
  test(`file drop event should not propagate to dialog parent`, async ({
    ...
    const getItem = () => getList().locator('[data-ui="Grid"] > div')
    ...
    // Try to drop the file with error handling
    try {
      // Drop the file
      await getList().dispatchEvent('drop', {dataTransfer})

      // Check if the item appears
      await expect(getItem()).toHaveCount(1, expectConfig)

      // Find and click the file button
      const getFileButton = () => page.getByText(fileName, {exact: true}).first()
      await expect(getFileButton()).toBeVisible(expectConfig)
      await getFileButton().click()

      // Verify dialog appears
      await expect(page.getByRole('dialog')).toBeVisible(expectConfig)
      ...
      // Verify item count is still 1
      await expect(getItem()).toHaveCount(1, expectConfig)
    } catch (e) {
      // If test fails, take a screenshot for debugging
      await page.screenshot({path: `test-results/file-drop-failure-${Date.now()}.png`})
      throw e
    }
  })
})
